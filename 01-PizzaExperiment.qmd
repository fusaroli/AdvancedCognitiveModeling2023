...
mod <- cmdstan_model("models/pizza_physics_model.stan", quiet = TRUE)
```
...
parameter estimation via Bayesian inference (Ch. 4), validation of that
inference (Ch. 5), and scaling to many participants (Ch. 6). The pizza stone
...
the right to touch real human data after passing the validation battery in Chapter 5.
