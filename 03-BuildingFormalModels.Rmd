...
*Note on Programming:* In R, we usually prefer vectorization (doing everything at once) over loops. However, cognitive models are inherently **sequential**: what I do now depends on what I just saw. Therefore, we use an explicit `if/else` logic here to mirror the cognitive process.
...
Now that we have our agents defined as functions, we can run a **Virtual Experiment**. We don't need to recruit participants; we can simulate thousands of trials in seconds.
