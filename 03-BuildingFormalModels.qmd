...
> We have verbal theories (Ch. 2). This chapter turns them into runnable code —
> **forward models** that generate data *from* parameters. This is a
> prerequisite for Ch. 5, where we reverse the direction: given observed data,
> we will *infer* the parameters. You cannot run that inversion without first
> knowing how to run the model forward.
...
* **Prepare for Model Fitting:** Appreciate why simulating models is a crucial step prior to estimating parameters from empirical data (covered in Chapter 5).
...
```{r ch04_implement_random_agent, fig.show='hold'}
...
> *Why run a virtual experiment instead of just testing one agent?* A single
> simulation is like flipping a coin once — you cannot tell if the coin is fair
> from one flip. Running 100 simulated agents lets us separate the signal (the
> model's actual tendency) from the noise (random variation in any one run). It
> also lets us sweep across parameter values (e.g., varying opponent bias from
> 0.5 to 1.0) to understand *when* WSLS works and when it fails — building
> intuition before we ever see real human data.
...
sim_data_file <- here("simdata", "ch03_scaled_simulations.rds")
...
if (regenerate_simulations || !file.exists(sim_data_file)) {
  ...
  saveRDS(df_scaled_vs_random, sim_data_file)
} else {
  df_scaled_vs_random <- readRDS(sim_data_file)
}
...
**Coming up in Chapter 5:** We reverse the arrow. The simulated data we just
created — sequences of choices generated by agents with *known* parameters —
become our test cases. We will learn to infer those parameters back from the
choices using Bayesian inference in Stan. This single-dataset fit is the
foundation; in Chapter 6, we will then stress-test the entire inference
procedure (prior predictive checks, parameter recovery, SBC, posterior
predictive checks) before the model is allowed anywhere near real human data.

::: {.callout-note title="Open Science and the Case for Transparent Implementations"}

You have just written your first formal cognitive model: a function that takes parameters, runs an algorithm, and produces data. It is tempting to treat this as a private computational tool — something you run, extract a result from, and move on. The history of cognitive modeling suggests this would be a mistake.

A persistent challenge in the field is what we might call the **hidden implementation problem**. Two research groups both claim to implement the same model — say, a Win-Stay-Lose-Shift agent, or later in the course, an exemplar model or a reinforcement learner. Their verbal descriptions match. Their equations look identical on paper. But their code differs in subtle ways: how they handle the first trial, whether they normalise probabilities before or after applying a noise parameter, how they deal with edge cases. The result is that "the same model" produces different predictions, and conflicting empirical conclusions are blamed on the data rather than the implementation. The field accumulates apparent contradictions that are really implementation artifacts.

The solution is not to standardise everyone onto a single package — though shared, collaboratively verified implementations are genuinely valuable. A pioneering example is the `catlearn` R package [@wills2017progress], which provides formally verified implementations of classic categorization models alongside canonical benchmark datasets, so that results from different labs are directly comparable. The broader principle extends to every domain of cognitive modeling: when implementations are open and documented, errors can be found and fixed; when they are private, the same errors propagate silently.

These course notes are designed as a contribution to this open-science ethos. Rather than handing you pre-built scripts to run, we ask you to implement each model yourself, document every assumption explicitly, validate the implementation against known inputs and outputs, and produce code you could share. You will not struggle with an equation in silence and then quietly shelve the script because you are not sure it is correct. You will build the habits — transparent code, documented choices, validated outputs — that make science cumulative rather than siloed. Every chapter that follows applies this principle: forward simulator, Stan model, diagnostic battery, and validation checks, all in open, reproducible code.
