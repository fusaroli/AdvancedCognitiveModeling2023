...
d1 <- d %>% 
  ...
  mutate(cumulativerate = lag(cumulativerate, 1),
         cumulativerate = ifelse(cumulativerate == 0, -3, ifelse(
           cumulativerate == 1, 3, logit_scaled(cumulativerate)) ))
...
We can see that the model has now estimated both the bias and the role of previous memory. Bias should reflect the bias in the setup (0.5 which in log odds is 0), and the beta coefficient for memory (roughly 0.8). More on quality checks of the models in the next chapter.
