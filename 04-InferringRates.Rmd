...
```{tikz, echo=FALSE, fig.cap="Binomial Model with Formulas and distributions", fig.ext = 'png', cache=TRUE}

\usetikzlibrary{positioning, shapes.geometric, arrows.meta, calc}

\begin{tikzpicture}[
    node distance=2.5cm,
    % Define the "stamper" pictures for the distributions
    pics/beta_dist/.style={
        code={
            % Axis
            \draw[->] (-0.6,0) -- (0.6,0);
            \draw[->] (-0.6,0) -- (-0.6,0.8);
            % The Curve (simplified hump)
            \draw[thick, blue] plot[domain=-0.5:0.5, smooth] (\x, {0.7*exp(-10*\x*\x)});
        }
    },
    pics/bar_dist/.style={
        code={
            ...
            \draw[->] (-0.6,0) -- (0.6,0);
            \draw[->] (-0.6,0) -- (-0.6,0.8);
            % The Bars (simplified histogram)
            \draw[thick, blue, ycomb] plot coordinates {(-0.3, 0.2) (0, 0.6) (0.3, 0.3)};
        }
    },
    % Standard Styles
    latent/.style={circle, draw=black, thick, minimum size=2cm, fill=white},
    observed/.style={rectangle, draw=black, thick, minimum size=2cm, fill=gray!10},
    stochastic/.style={->, >={Latex[length=3mm]}, thick}
]

    % --- NODES ---

    % 1. Rate (Theta) - Showing a Beta Distribution
    \node[latent] (theta) {}; 
    \draw (theta.center) pic {beta_dist}; % "Stamp" the plot inside
    \node[above=0.1cm of theta] {$\theta \sim \text{Beta}(1,1)$};

    % 2. Right (Outcomes) - Showing a Bar Chart
    \node[observed, below=of theta] (k) {};
    \draw (k.center) pic {bar_dist}; % "Stamp" the plot inside
    \node[right=0.2cm of k] {$\text{Right} \sim \text{Binomial}(\theta, n)$};

    % 3. Trials (n) - Just a number
    \node[observed, minimum size=1cm, left=of k] (n) {$n$};
