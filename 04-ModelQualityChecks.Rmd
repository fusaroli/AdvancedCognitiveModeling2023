...
## Then we need a prior sensitivity analysis
...
MemoryAgent_f <- function(cumulativerate){
    choice = rbinom(1, 1, inv_logit_scaled(0.9 * logit_scaled(cumulativerate)))
  return(choice)
}

d1$Self[1] <- RandomAgent_f(1, 0.5)

for (i in 2:trials) {
  d1$Self[i] <- MemoryAgent_f(d1$cumulativerate[i])
}
...
## Now prior sensitivity check for the memory model
