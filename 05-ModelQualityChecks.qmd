# Assessing Model Quality {#sec-model-quality-checks}

```{r ch06_setup, include=FALSE}
...
> Ch. 5 fit a Stan model to one simulated dataset. Obtaining a posterior is
...
In Chapter 5, we learned how to write Stan code and infer parameters for a single simulated dataset. However, obtaining a posterior distribution does not mean the model is correct or the inference is reliable.
...
* **Calibrate Uncertainty (Simulation-Based Calibration Checks):** Use SBC to guarantee your computational engine is unbiased and your posterior uncertainties are mathematically sound.
...
**Note on terminology:** In Ch. 5 we used "parameter recovery" loosely to mean
...
In the previous chapters, we journeyed from observing behavior (Chapter 2), formalizing strategies (Chapter 3), to fitting models and estimating parameters (Chapter 5).
...
Let's start by loading the necessary packages and the fitted model object from Chapter 5. This avoids re-running the fitting process unnecessarily and ensures consistency.

```{r ch06_load_packages, warning = F, message = F}
...
::: {.callout-tip title="Stan Design Pattern — Priors as Data"}
Notice that we declare the prior hyperparameters (`prior_theta_mu`, `prior_theta_sigma`) in the **`data` block** rather than hardcoding them in the `model` block. This separation has two concrete benefits:

1. **No recompilation when exploring priors.** Changing a hardcoded `normal(0, 1.5)` to `normal(0, 2.0)` in Stan requires recompiling the C++ model binary (~10–30 seconds). Passing them as data means you change a single R list entry and re-run instantly.
2. **SBC priors can differ from the fitted model's priors.** When SBC simulates from the prior, it draws hyperparameters from the same distribution specified in the data list. Keeping this in sync is trivial when priors are data; it is easy to introduce accidental mismatches when they are hardcoded.

We use this pattern for every model with tunable priors from this chapter onward.
...
::: {.callout-tip title="Stan Efficiency Rule 2 — Conditional Diagnostics: The `run_diagnostics` Flag"}

The six-phase validation pipeline requires **thousands of Stan fits** during SBC (Simulation-Based Calibration Checks). Each SBC iteration only needs to verify posterior calibration — it does **not** need:

- PPC draws (`y_rep`) — these require one `bernoulli_rng()` call per observation per draw
- Pointwise log-likelihoods (`log_lik`) — needed for LOO-CV, but LOO is not run on SBC fits
- Prior predictive samples — only needed for the visual prior check, not for calibration

Generating these quantities inside `generated quantities` on every SBC iteration wastes substantial compute. We gate them with a single integer flag in the data block:

```stan
int<lower=0, upper=1> run_diagnostics;  // 1 = full diagnostics; 0 = calibration only
```

```stan
generated quantities {
  vector[N_trials] log_lik;
  array[N_trials] int y_rep;
  if (run_diagnostics) {
    for (i in 1:N_trials) {
      log_lik[i] = bernoulli_logit_lpmf(y[i] | theta_logit);
      y_rep[i]   = bernoulli_logit_rng(theta_logit);
    }
  }
}
```

Then in R, the main validation fit uses `run_diagnostics = 1` and SBC backends use `run_diagnostics = 0`:

```r
stan_data_main <- list(..., run_diagnostics = 1L)   # single model check
stan_data_sbc  <- list(..., run_diagnostics = 0L)   # every SBC iteration
```

**Why it matters at scale.** For a model with 120 trials and 100 SBC datasets × 4 chains × 1000 draws: gating `log_lik` and `y_rep` eliminates 100 × 4 × 1000 × 120 ≈ **48 million** log-probability evaluations, typically cutting SBC wall-clock time by 20–40%.

Note that **prior samples** (e.g., `bias_prior = normal_rng(0, 0.3)`) are cheap scalar draws and can remain ungated — they don't loop over observations.

We introduced this flag in simplified form in Chapter 4's memory model. From this chapter onward it appears in every Stan model in the book.

:::

::: {.callout-tip title="Stan Efficiency — Post-Hoc Generated Quantities with `$generate_quantities()`"}

The `run_diagnostics` flag trades off between fitting speed and diagnostic richness.
A complementary pattern avoids the trade-off entirely: **fit once with `run_diagnostics = 0`,
then replay the saved draws through the `generated quantities` block on demand** using
`$generate_quantities()`.

```r
# Fast fit — no log_lik or y_rep computed during sampling
fit_fast <- mod$sample(data = list(..., run_diagnostics = 0L), ...)

# Later: compute log_lik and y_rep from the saved draws without re-running MCMC
...
`$generate_quantities()` runs only the `generated quantities` block — it never touches the
sampler. This means:

- **LOO and PPC can be deferred** until you actually need them, without re-running MCMC.
- **SBC backends always run with `run_diagnostics = 0`** and you call `$generate_quantities()` only on the subset of fits you want to inspect.
- The saved `CmdStanFit` object from a previous session can be replayed by loading it with `readRDS()` and passing it as `fitted_params`.

The only requirement is that the `data` list passed to `$generate_quantities()` matches the
data block of the compiled model — the same model object must be used for both calls.

:::

```{r ch06_simple_biased_agent_gq, warning = F, message = F}
...
"
stan_file_gq <- file.path(stan_model_dir, "ch07_SimpleBernoulli_GQ.stan")
...
model_file_gq <- file.path(stan_results_dir, "ch07_SimpleBernoulli_GQ.rds")

# Generate the required data

# Load the simulation data used in Chapter 5
sim_data_file <- file.path(sim_data_dir, "ch06_randomnoise.csv") 
d <- read_csv(sim_data_file, show_col_types = FALSE) 

# Subset data for the specific agent fitted in Chapter 5 (rate=0.8, noise=0)
...
.run_sampling <- regenerate_simulations || !file.exists(model_file_gq)

if (!.run_sampling) {
  fit_gq <- readRDS(model_file_gq)
  # Detect stale cache: if y_post_rep is missing, the model was saved before PPC was added
  .has_ppc <- tryCatch({ fit_gq$draws("y_post_rep"); TRUE }, error = function(e) FALSE)
  if (!.has_ppc) .run_sampling <- TRUE
}
...
observed_sum <- sum(observed_data_list$y) # Total number of 'right' choices in the actual observed data
...
* Simulate Replications: Generate a synthetic dataset $y_{\text{rep}}^{(s)}$ of the exact same dimensions as the empirical data, using the data model:$$y_{\text{rep}}^{(s)} \sim \text{Bernoulli}(\theta^{(s)})$$
...
We extract the replicated datasets (y_post_rep) generated simultaneously within the Stan generated quantities block. We will then systematically compare the test statistic distributions $T(y_{\text{rep}})$ against our empirical ground truth $T(y)$.

```{r ch06_posterior_predictive_checks, warning = F, message = F}
...
ppc_posterior_ggplot <- ggplot(ppc_df, aes(x = Replicated_Sum)) +
  ...
  annotate(
    ...
    vjust = 2, hjust = if(observed_sum > N_trials/2) 1.1 else -0.1, 
...
observed_max_run <- max_run_length(observed_data_list$y)
...
y_prior_rep <- fit_gq$draws("y_prior_rep", format = "matrix")
...
```{r ch06_prior_posterior_update, warning = FALSE, message = FALSE}
...
## Phase 5: Prior Sensitivity Analysis - Do Priors Matter Too Too Much?
...
```{r ch06_manual_sensitivity_analysis, warning = F, message = F}
# We test 3 location parameters (means) and 10 scale parameters (SDs)
prior_grid <- tidyr::expand_grid(
  prior_mu = c(-1.5, 0, 1.5), 
  prior_sigma = seq(0.5, 3.0, length.out = 10)
)

# Standardized filename variable
sensitivity_draws_file <- file.path(here(stan_results_dir), "ch07_sensitivity_draws.rds")
...
if (regenerate_simulations || !file.exists(sensitivity_draws_file)) {
  ...
  fit_and_extract <- function(prior_mu, prior_sigma) {
    # Dynamically inject the perturbed parameters into the data list
    current_data_list <- observed_data_list
    current_data_list$prior_theta_mu <- prior_mu
    current_data_list$prior_theta_sigma <- prior_sigma 
...
p_sensitivity_2d <- ggplot(sensitivity_draws, aes(x = theta_prob, group = factor(prior_sigma), color = prior_sigma)) +
  geom_line(stat = "density", alpha = 0.8, linewidth = 0.8) + 
  geom_vline(xintercept = true_theta_prob, color = "black", linetype = "dashed", linewidth = 1) + 
  facet_wrap(~ prior_mu, labeller = labeller(prior_mu = function(x) paste0("Prior Mean (Logit): ", x))) +
...
sensitivity_summary <- sensitivity_draws |> 
  dplyr::group_by(prior_mu, prior_sigma) |> 
  dplyr::summarize(
...
p_sensitivity_trajectory <- ggplot(sensitivity_summary, aes(x = prior_sigma, y = post_mean)) +
  ...
  geom_ribbon(aes(ymin = lower_95, ymax = upper_95, fill = prior_mu_label), alpha = 0.15) +
  
  # The 68% Credible Interval (~1 SD, Inner Ribbon, More Opaque)
  geom_ribbon(aes(ymin = lower_68, ymax = upper_68, fill = prior_mu_label), alpha = 0.35) +
  
  # The posterior mean trajectory
  geom_line(aes(color = prior_mu_label), linewidth = 1) +
  geom_point(aes(color = prior_mu_label), size = 2) +
  
  # Structural faceting
  facet_wrap(~ prior_mu_label) +
...
```{r ch06_priorsense_analysis, warning = F, message = F}
...
```{r ch06_parameter_recovery_grid}
recovery_grid <- tidyr::expand_grid(
  N_trials = c(30, 60, 120, 240),
  true_theta = c(0.5, 0.7, 0.9)
)

recovery_file <- file.path(stan_results_dir, "ch07_recovery_draws.rds")
...
if (regenerate_simulations || !file.exists(recovery_file)) {
  ...
  run_recovery <- function(N_trials, true_theta) {
    ...
    sim_y <- rbinom(n = N_trials, size = 1, prob = true_theta)
...
### Phase 6b. Simulation-Based Calibration Checks (SBC)
...
If our inference systematically generates posterior distributions that are too narrow (overconfident) or too wide (underconfident), our 95% Credible Intervals are fictions. To formally investigate whether our posterior geometry is calibrated across the entire prior space, we must execute Simulation-Based Calibration Checks (SBC).
...
2. Simulate a dataset from the data model: $\tilde{y} \sim p(y \mid \tilde{\theta})$
...
```{r ch06_sbc_calibration, warning = FALSE, message = FALSE}

sbc_results_file <- file.path(stan_results_dir, "ch07_sbc_results.rds")
...
if (regenerate_simulations || !file.exists(sbc_results_file)) {
  ...
  sbc_generator_fn <- function() {
    ...
    N_trials <- 120
    y_sim    <- rbinom(N_trials, size = 1, prob = theta_true)
...
```{r ch06_sbc_recovery_plot, warning = FALSE, message = FALSE}
...
```{r ch06_sbc_precision_analysis, warning = FALSE, message = FALSE}
# ---------------------------------------------------------------------------
...
precision_stats <- purrr::map_dfr(trial_grid_precision, function(n_t) {
  ...
  precision_file <- file.path(stan_results_dir,
                          sprintf("ch07_sbc_precision_n%d.rds", n_t))
...
2. Simulate a new synthetic dataset $\tilde{y}$ from the data model: $\tilde{y} \sim p(y \mid \tilde{\theta})$.
...
generate_psbc_data_theta <- function(n_sims = 1000) {
  ...
  for (i in 1:n_sims) {
    ...
    y_sim <- rbinom(observed_data_list$N_trials, size = 1, prob = true_prob)
    
    gen_list[[i]] <- list(
      N_trials          = observed_data_list$N_trials,
      y                 = y_sim,
      prior_theta_mu    = 0,
      prior_theta_sigma = 1.5,
      run_diagnostics   = 0
...
psbc_results_file <- file.path(stan_results_dir, "ch07_psbc_results.rds")
...
The biased agent is a clean single-parameter model. Real cognitive models are messier. This section applies the same six-phase workflow to the memory model from Ch. 5 — a model with two parameters (`bias` and `beta`) where the choice probability on each trial depends on the opponent's cumulative history.
...
"
stan_file_mem_gq <- file.path(stan_model_dir, "ch07_Memory_GQ.stan")
...
observed_data_list_mem <- list(
  N_trials = N_trials,
  y = d_memory_obs$choice,
  other = d_memory_obs$other_choice,
  prior_bias_mu = 0, prior_bias_sigma = 0.5,
  prior_beta_mu = 0, prior_beta_sigma = 1.0,
  run_diagnostics = 1
)

model_file_mem_gq <- file.path(stan_results_dir, "ch07_Memory_GQ.rds")
...
if (regenerate_simulations || !file.exists(model_file_mem_gq)) {
  ...
  fit_mem_gq <- mod_mem_gq$sample(
    ...
    chains = 4,
    parallel_chains = max(1, parallel::detectCores() - 2),
    iter_warmup = 1000,
    iter_sampling = 2000,
    refresh = 0,
...
update_long <- draws_mem %>%
  dplyr::select(bias, beta, bias_prior, beta_prior) %>%
...
cov_collapse_file <- file.path(stan_results_dir, "ch07_cov_collapse_draws.rds")
...
```{r ch06_fit_memory_reversal}
model_file_reversal <- file.path(stan_results_dir, "ch06_Memory_Reversal.rds")
...
reversal_data_file <- file.path(sim_data_dir, "ch07_reversal_data.rds")
...
cat("Load in Ch. 7 and Ch. 8 with: readRDS(here('simdata', 'ch07_reversal_data.rds'))\n")
```

Finally, we run Simulation-Based Calibration Checks. We use the SBC package to guarantee that our logit(memory) parameterization doesn't introduce hidden biases in the sampler's rank statistics.

```{r ch06_memory_sbc}
sbc_mem_results_file <- here::here("simmodels", "ch06_memory_sbc_results.rds")

if (regenerate_simulations || !file.exists(sbc_mem_results_file)) {
  ...
  sbc_memory_generator_fn <- function() {
    ...
    list(
      ...
      generated = list(N_trials = 120, y = sim$choice, other = opp_choices,
                       prior_bias_mu = 0, prior_bias_sigma = 0.5,
                       prior_beta_mu = 0, prior_beta_sigma = 1.0,
                       run_diagnostics = 0L)
    )
  ...
  saveRDS(sbc_mem_results, sbc_mem_results_file)
} else {
  sbc_mem_results <- readRDS(sbc_mem_results_file)
}
...
The reversal learning dataset generated in this chapter (`simdata/ch07_reversal_data.rds`) is the running example for the next two chapters. It is well-identified, validated against all six checks, and ready to be extended.

**Coming up in Chapter 7:** We take the memory model — now validated on the reversal paradigm — and scale it to many simulated participants using a hierarchical (multilevel) model. The central question shifts from "can we estimate parameters for one agent?" to "how do individuals vary, and how do we share information across them?"
