...
# Load the validated memory model, LOO score, and reversal schedule from Ch. 6.
# These were saved by the final chunk of Ch. 6.
ch6_exports_file <- here::here("simmodels", "06_memory_exports_for_ch7.rds")

if (file.exists(ch6_exports_file)) {
  ch6_exports       <- readRDS(ch6_exports_file)
  opponent_reversal <- ch6_exports$reversal_schedule  # 240-trial binary vector
  loo_memory_ch6    <- ch6_exports$loo_memory          # LOO from Ch. 6 memory fit
  cat("Ch. 6 exports loaded.\n")
  cat("  Reversal schedule:", length(opponent_reversal), "trials\n")
  cat("  Ch. 6 memory ELPD:",
      round(loo_memory_ch6$estimates["elpd_loo", "Estimate"], 1), "\n")
} else {
  warning(paste(
    "Ch. 6 exports not found at", ch6_exports_file,
    "\nFalling back to a freshly generated reversal schedule.",
    "\nRun Ch. 6 first to ensure consistency across chapters."
  ))
  # Reproducible fallback so Ch. 7 can knit independently
  set.seed(2025)
...
> uses PSIS-LOO cross-validation and requires the validated fits from Ch. 6.
> The reversal learning design introduced in Ch. 5 and used in Ch. 6 is
...
We simulate two populations of agents playing against a shared opponent using the **reversal design** first validated in Ch. 5 and used throughout Ch. 6: the opponent plays 80% right for the first half of the session, then switches to 20% right for the second half. We use the exact sequence loaded from Ch. 6 above (`opponent_reversal`), taking the first 120 trials (60 + 60) for this chapter's shorter sessions.
...
simulate_model_recovery_data <- function(
  ...
  # If a fixed sequence is supplied (from Ch. 6), use it directly so that
...
if (regenerate_simulations || !file.exists(sim_data_path)) {
  sim_data <- simulate_model_recovery_data(
    opponent_choices = opponent_reversal   # Use Ch. 6 reversal schedule
  )
...
For context, the Ch. 6 memory model (fitted to the full 50-agent reversal dataset with 80–240 trials per agent) had
`r if (!is.null(loo_memory_ch6)) paste0("ELPD = ", round(loo_memory_ch6$estimates["elpd_loo","Estimate"], 1), " (SE = ", round(loo_memory_ch6$estimates["elpd_loo","SE"], 1), ")") else "ELPD = [load Ch. 6 exports to see this value]"`.
The Ch. 7 memory-on-memory fit uses a shorter, fixed-length session (120 trials vs. 80–240), so its absolute ELPD is not directly comparable — but the sign and direction of all comparisons should be consistent.
...
- The **reversal design** (validated in Ch. 5, extended in Ch. 6) was
...
Chapter 8 introduces a new model type — mixture models — that extends the
