...
* **Explain the failure mode of in-sample fit**: Understand why simply choosing the model that fits the training data best can be misleading.
...
* **Define ELPD rigorously**: state the Expected Log Predictive Density as a formal target quantity and connect it to the log-score proper scoring rule.

* **Implement PSIS-LOO**: compute ELPD estimates from existing Stan fits using `loo::loo()`, identify Pareto-$\hat{k}$ diagnostics, and apply `loo::loo_moment_match()` for problematic observations.

* **Characterize influential observations**: identify *why* specific observations have high $\hat{k}$, linking influence back to the cognitive structure of the experiment.

* **Diagnose LOO-PIT calibration correctly**: understand why the standard ECDF visual can be too lenient for complex hierarchical models; compute the $\hat{p}_{\text{loo}}/N$ complexity ratio; implement the POT-C formal test that remains valid when LOO-PIT values are correlated.

* **Interpret `loo_compare()` output**: read `elpd_diff` and `se_diff` correctly and reason about the $|\text{elpd\_diff}|/\text{se\_diff}$ ratio.

* **Compute Bayesian stacking weights**: use `loo::loo_model_weights()` to assign ensemble weights to competing models.

* **Execute model recovery**: design a 2x2 simulation study where the true generating model is known and verify that PSIS-LOO identifies it correctly.

* **Identify when LOO is invalid**: recognize that trial-level LOO is inappropriate for sequentially dependent data and describe the Leave-Future-Out remedy.
...
1. **Proper Scoring**: In standard machine learning, we might just calculate the mean squared error or accuracy on the test fold. But Bayesian models output *probability distributions*, not just point estimates. We need an appropriate metric for evaluating the quality of a predictive distribution. 
...
# 1. Simulate 1000 raw log-importance weights (negative log-likelihoods)
# Most are normal, but we inject a heavy right tail of extreme outliers
log_w_raw <- c(rnorm(950, mean = 0, sd = 1), seq(3, 15, length.out = 50))

# 2. Apply the ACTUAL PSIS algorithm from the loo package!
...
# Smoothed weights (psis() returns them already log-normalized)
w_smooth_norm <- exp(as.numeric(psis_result$log_weights))
...
# We will zoom in on the top 150 weights to see the tail clearly.
...
We just saw a successful single instance of model recovery: for our specific simulated dataset, loo_compare() correctly preferred the true generating model in both cases. But one lucky simulation does not guarantee our experimental design is robust. 
...
In the matrix above, when the true model was "Biased," LOO correctly identified it 95% of the time. When the true model was "Memory," LOO correctly identified it 85% of the time. (Complex models are often harder to recover because of the stringent ELPD penalties on flexibility).

### Designing the Experiment: Trials, Participants, and Task Structure

How do we achieve this beautiful matrix? The most obvious knobs to turn are sample sizes. In hierarchical cognitive modeling, we actually have two distinct sample sizes to worry about: the number of trials per participant ($N$), and the number of participants ($J$). 

These two dimensions govern different types of statistical power:
* **More Trials ($N$):** Increases our ability to accurately estimate *individual-level* parameters. If we only have 30 trials, we cannot confidently detect the subtle influence of the Memory parameter within a single person.
* **More Participants ($J$):** Increases our ability to accurately estimate the *population-level* distribution. 

There is often a practical trade-off here (e.g., your budget only allows for 6,000 total trials, so do you run 100 people for 60 trials, or 50 people for 120 trials?). 

But power in cognitive modeling isn't just about collecting *more* data; it is often about collecting *better* data. We can vary the experimental setup itself. For example, in our Matching Pennies simulation, we explicitly forced the opponent to switch from an 80%-right bias to a 20%-right bias halfway through the game. We did this because a constant 80% bias might not produce enough variance in the memory signal to confidently separate a Biased agent from a Memory agent. 

To conduct a formal **power analysis for model comparison**, we systematically repeat our recovery pipeline across a grid of design choices (e.g., varying trial counts and participant counts). We then plot the recovery accuracy as a 2D heatmap to find the optimal experimental design.

```{r 07-model-recovery-tradeoff, fig.height=5, fig.width=7}
# --- Conceptual Power Trade-off Matrix ---
# Create a grid of trial counts and participant counts
power_grid <- expand_grid(
  N_Trials = c(30, 60, 90, 120, 150),
  N_Participants = c(20, 40, 60, 80, 100)
) |>
  mutate(
    # Conceptual mathematical function to simulate power:
    # Diminishing returns for both, but trials are slightly more critical 
    # for identifying the memory parameter within individuals.
    Accuracy = 1 - exp(-0.02 * N_Trials) * exp(-0.025 * N_Participants),
    # Cap at realistic max and add a little noise/rounding for realism
    Accuracy = pmin(0.99, round(Accuracy, 2))
  )

ggplot(power_grid, aes(x = factor(N_Trials), y = factor(N_Participants), fill = Accuracy)) +
  geom_tile(color = "white", linewidth = 1) +
  geom_text(aes(label = scales::percent(Accuracy, accuracy = 1)), 
            size = 5, fontface = "bold", 
            color = ifelse(power_grid$Accuracy > 0.85, "white", "black")) +
  scale_fill_viridis_c(option = "mako", direction = -1, limits = c(0, 1), 
                       labels = scales::percent) +
  labs(title = "Model Recovery Power: Trials vs. Participants",
       subtitle = "Accuracy of recovering the Memory model across different experimental designs",
...
Looking at this trade-off matrix, we can definitively tell our grant agency or ethics board: "To reliably distinguish between Biased and Memory strategies (e.g., >80% accuracy), we cannot rely on large participant counts alone. Running 100 participants for only 30 trials yields inadequate power (53%). We must ensure sufficient within-subject data, requiring a minimum design of roughly 50 participants completing 120 trials each."
...
To assess if our model comparison is well-calibrated, we treat the stacking weights as a measure of predictive confidence. If a model is assigned a weight of 0.70, it should "be" the true model in roughly 70% of similar cases.
