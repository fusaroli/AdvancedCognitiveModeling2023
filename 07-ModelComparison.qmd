...
# Load the validated memory model, LOO score, and reversal schedule from Ch. 7.
# These were saved by the final chunk of Ch. 7.
ch07_exports_file <- here::here("simmodels", "ch07_memory_exports_for_ch7.rds")

if (file.exists(ch07_exports_file)) {
  ch07_exports       <- readRDS(ch07_exports_file)
  opponent_reversal <- ch07_exports$reversal_schedule  # 240-trial binary vector
  loo_memory_ch6    <- ch07_exports$loo_memory          # LOO from Ch. 7 memory fit
  cat("Ch. 7 exports loaded.\n")
  cat("  Reversal schedule:", length(opponent_reversal), "trials\n")
  cat("  Ch. 7 memory ELPD:",
      round(loo_memory_ch6$estimates["elpd_loo", "Estimate"], 1), "\n")
} else {
  warning(paste(
    "Ch. 7 exports not found at", ch07_exports_file,
    "\nFalling back to a freshly generated reversal schedule.",
    "\nRun Ch. 7 first to ensure consistency across chapters."
  ))
  # Reproducible fallback so Ch. 8 can knit independently
  set.seed(2025)
  opponent_reversal <- c(rbinom(120, 1, 0.8), rbinom(120, 1, 0.2))
  loo_memory_ch6    <- NULL
}
```

> **📍 Where we are in the Bayesian modeling workflow:**
> Chs. 1–6 built models, validated them, and scaled them to populations.
> Every model passed a six-phase quality battery before fitting real data.
> This chapter asks the next question: when two validated models both fit
> the data, which one better *predicts* data it has never seen? The answer
> uses PSIS-LOO cross-validation and requires the validated fits from Ch. 7.
> The reversal learning design introduced in Ch. 6 and used in Ch. 7 is
> essential here too — it is the design that makes the two models
...
### Historical Context: From Fit to Prediction — The History of Model Selection

How to choose among competing models is one of the oldest unsolved problems in statistics. The modern answer — estimate out-of-sample predictive accuracy — is less obvious than it sounds, and the path to it passed through several influential wrong turns.

#### The In-Sample Era: Likelihood and R²

The classical approach was to ask which model assigns the highest probability to the observed data: the maximum likelihood. This criterion was the backbone of hypothesis testing (Fisher, 1922; Neyman & Pearson, 1933) and dominated empirical science throughout the twentieth century. Its failure mode was known — a model with enough free parameters can fit any finite dataset perfectly — but was treated as a matter of scientific judgment rather than a formal problem. The coefficient of determination $R^2$ shared the same flaw: it increases monotonically with model complexity and therefore cannot serve as a principled selection criterion.

#### Information Criteria: AIC and BIC

The first principled solution came from information theory. @akaike1974new derived the **Akaike Information Criterion** (AIC) by asking a precise question: what is the expected Kullback-Leibler divergence between the true data-generating distribution and the distribution implied by a fitted model, evaluated on *new* data from the same process? The answer, to leading order, is $-2\log\hat{L} + 2k$, where $\hat{L}$ is the maximized likelihood and $k$ is the number of free parameters. The penalty term $2k$ is a correction for the optimism of in-sample fit: a model that uses $k$ extra parameters gains approximately $2k$ units of log-likelihood purely by fitting noise, and AIC subtracts this back out.

@schwarz1978estimating derived the **Bayesian Information Criterion** (BIC) from a different starting point — approximating the log marginal likelihood under a diffuse prior — and obtained a penalty of $k \log n$ that grows with sample size and more aggressively penalises complexity. AIC and BIC dominated applied statistics through the 1980s and 1990s, but both inherit a key limitation: they are computed on the training data and require asymptotic approximations whose validity in small, heterogeneous cognitive datasets is difficult to verify.

#### Cross-Validation and the Equivalence Theorem

@stone1977asymptotic proved a result that initially seemed surprising: AIC and leave-one-out cross-validation (LOO-CV) are **asymptotically equivalent**. Cross-validation — holding out each observation in turn, refitting on the rest, and evaluating predictive accuracy on the held-out point — gives the same answer as AIC in large samples, but without requiring asymptotic approximations or a count of free parameters. This equivalence is important because cross-validation is model-agnostic: it works on any model that can generate predictions, whether or not its parameter count is well-defined (as is the case for hierarchical Bayesian models where effective complexity is data-dependent).

#### PSIS-LOO: Making Cross-Validation Affordable

The practical obstacle to LOO-CV was computational: for a model fit to $N$ observations, exact LOO requires $N$ separate refits. @vehtari2017practical solved this with **Pareto-Smoothed Importance Sampling LOO** (PSIS-LOO). The key insight is that a model fit to all $N$ observations already contains, implicitly, the posterior for each leave-one-out model — the left-out observation simply receives zero weight. Importance sampling reweights the full-data posterior to approximate the leave-one-out posterior, and Pareto-smoothing of the importance weights stabilises the estimator and, crucially, provides a per-observation diagnostic (the $\hat{k}$ statistic) that flags when the approximation is unreliable. This diagnostic turned model comparison from a black-box number into an interpretable workflow: high $\hat{k}$ observations are not just nuisances but windows into the structure of the data that the model is struggling to predict.

The arc from AIC (1974) to PSIS-LOO (2017) is a story about a single conceptual shift — from *how well does the model fit what we have seen?* to *how well does the model predict what we have not yet seen?* — being made progressively more rigorous, more computationally feasible, and more transparent about its own failure modes.
...
ggplot(df_fit, aes(x = x, y = y)) +
  ...
  labs(title    = "The Overfitting Trap: In-Sample Fit Favours Complexity",
       subtitle = "Degree-10 polynomial fits training data perfectly but would fail on new data. Degree-2 (true DGP) generalizes best.",
...
> *Why learn the mechanics before using `loo()`?* The `loo()` function is
> one line of R. But knowing what it is doing — reweighting posterior draws
> to approximate leave-one-out predictions, then checking whether those
> weights are stable via the Pareto tail — lets you diagnose failures and
> interpret the Pareto-$\hat{k}$ diagnostic as a *cognitive* signal (which
> observations is the model surprised by?) rather than just a warning light.
> The under-the-hood demonstration on the next page is worth reading once
> carefully; after that, `loo()` will never be a black box.
...
Why this design? If the opponent plays a constant bias, the memory agent's internal running average quickly settles near that value and stays flat.
Behaviorally, an agent responding to a flat memory signal is mathematically indistinguishable from a biased agent — both produce a fixed right-leaning rate. The reversal forces the memory signal to change sign at trial 61: memory agents reverse their behavior while biased agents do not. This contrast is what makes the two models theoretically separable, and why Ch. 6 discovered that a constant-bias design made `bias` and `beta` unidentifiable.
...
simulate_model_recovery_data <- function(
  ...
  # Reversal block design: 80% right for t 1:60, 20% right for t 61:120.
  # If a fixed sequence is supplied (from Ch. 7), use it directly so that
  # all chapters share the same opponent history.
  if (!is.null(opponent_choices)) {
    stopifnot(length(opponent_choices) >= n_trials)
    opp_choices <- opponent_choices[seq_len(n_trials)]
  } else {
    opp_choices <- c(
      rbinom(60, 1, 0.8),
...
sim_data_path <- here::here("simdata", "ch08_model_recovery_data.rds")

if (regenerate_simulations || !file.exists(sim_data_path)) {
  sim_data <- simulate_model_recovery_data(
    opponent_choices = opponent_reversal   # Use Ch. 7 reversal schedule
  )
...
bind_rows(d_biased, d_memory) |>
  ...
  labs(title    = "Cumulative Choice Rates: Biased vs. Memory Agents",
       subtitle = "Both populations play the reversal design (80% right → 20% right at trial 61). Memory agents shift; biased agents don't.",
...
stan_file_biased <- here::here("stan", "ch07_multilevel_biased_ncp.stan")
...
stan_file_memory <- here::here("stan", "ch07_multilevel_memory_ncp.stan")
...
> own data tells us the model works — but it cannot tell us whether ELPD
> actually discriminates. For model comparison to be meaningful, the wrong
> model must score *worse* than the right one. Fitting all four combinations
> gives us a 2×2 confusion matrix: if PSIS-LOO correctly selects the
> generating model in each column, the method has recovered the truth. If it
> selects the wrong model, that is scientifically informative too — it tells
> us the experimental design is not discriminative enough at this sample size.
...
map_dfr(names(fits), function(label) {
  ...
  knitr::kable(caption = paste(
    "Pre-comparison diagnostics. Required: Divergences = 0, R-hat < 1.01, ESS > 400.",
    "Misspecified fits (Memory->Biased, Biased->Memory) may show slightly worse ESS",
    "because the model is searching for structure that does not exist in the data.",
...
loo_b_on_b <- get_raw_loo(fit_b_on_b, here::here("simmodels", "ch08_loo_raw_b_on_b.rds"))
loo_m_on_b <- get_raw_loo(fit_m_on_b, here::here("simmodels", "ch08_loo_raw_m_on_b.rds"))
loo_b_on_m <- get_raw_loo(fit_b_on_m, here::here("simmodels", "ch08_loo_raw_b_on_m.rds"))
loo_m_on_m <- get_raw_loo(fit_m_on_m, here::here("simmodels", "ch08_loo_raw_m_on_m.rds"))
...
summarise_pareto_k <- function(loo_obj, label) {
  ...
  tibble(Combination=label, N=n,
         k_below_05=sum(k<0.5), k_ch06_07=sum(k>=0.5&k<0.7),
         k_ch08_10=sum(k>=0.7&k<1), k_above_10=sum(k>=1),
         Max_k=round(max(k),3),
...
agent_estimates <- fit_m_on_m$draws("indiv_params") |>
  ...
  dplyr::select(agent_id, metric, estimate_label) |>
...
loo_b_on_b <- apply_moment_match_if_needed(loo_b_on_b, fit_b_on_b, "Biased->Biased", here::here("simmodels", "ch08_loo_mm_b_on_b.rds"))
loo_m_on_b <- apply_moment_match_if_needed(loo_m_on_b, fit_m_on_b, "Memory->Biased", here::here("simmodels", "ch08_loo_mm_m_on_b.rds"))
loo_b_on_m <- apply_moment_match_if_needed(loo_b_on_m, fit_b_on_m, "Biased->Memory", here::here("simmodels", "ch08_loo_mm_b_on_m.rds"))
loo_m_on_m <- apply_moment_match_if_needed(loo_m_on_m, fit_m_on_m, "Memory->Memory", here::here("simmodels", "ch08_loo_mm_m_on_m.rds"))
```
...
}", here::here("stan", "ch08_biased_kfold.stan"))
...
}", here::here("stan", "ch08_memory_kfold.stan"))

mod_biased_kfold <- cmdstan_model(here::here("stan", "ch08_biased_kfold.stan"))
mod_memory_kfold <- cmdstan_model(here::here("stan", "ch08_memory_kfold.stan"))
cat("Both K-fold Stan models compiled.\n")

kfold_path <- here::here("simmodels","ch08_kfold_comparison.rds")
K <- 5; set.seed(21)
...
complexity_table <- pareto_k_table |>
  dplyr::select(Combination, N, p_hat_loo, ratio) |>
...
> *Why ELPD/LOO rather than Bayes Factors?* Bayes Factors compare the
> marginal likelihood of the data under each model — an elegant idea, but
> one that requires integrating over the *entire prior*. For hierarchical
> models, this integral is computationally intractable and the result is
> exquisitely sensitive to the exact width of every prior. PSIS-LOO avoids
> this by focusing entirely on predictive accuracy for held-out observations
> — a criterion that is robust to prior choices and provides the Pareto-$\hat{k}$
> as a built-in reliability diagnostic. The trade-off is philosophical: LOO
> measures pragmatic predictive utility, not the probability that a model is
> true. For most applied cognitive modeling questions, pragmatic predictive
> utility is what we actually care about.
...
- **Biased data:** The memory model is slightly worse, but the ratio is
  likely below 2 — *inconclusive*. This is the correct scientific answer:
  when data come from a purely biased agent, a memory model with $\beta \approx 0$
  produces indistinguishable predictions. The PSIS-LOO score is not broken;
  it is honestly reporting that the simpler model is sufficient here.
- **Memory data:** The biased model is decisively worse, with a ratio well
  above 4 — *clear preference*. The reversal at trial 61 produces behavioral
  changes that the biased model cannot track, and PSIS-LOO detects this.

This is **successful model recovery**: the method correctly identifies the
true generating model in the column where detection is possible, and honestly
reports uncertainty in the column where the models overlap. Both outcomes are
...
For context, the Ch. 7 memory model (fitted to the full 50-agent reversal dataset with 80–240 trials per agent) had
`r if (!is.null(loo_memory_ch6)) paste0("ELPD = ", round(loo_memory_ch6$estimates["elpd_loo","Estimate"], 1), " (SE = ", round(loo_memory_ch6$estimates["elpd_loo","SE"], 1), ")") else "ELPD = [load Ch. 7 exports to see this value]"`.
The Ch. 8 memory-on-memory fit uses a shorter, fixed-length session (120 trials vs. 80–240), so its absolute ELPD is not directly comparable — but the sign and direction of all comparisons should be consistent.
...
power_grid <- expand_grid(
  ...
  dplyr::select(N_Trials, N_Participants, F1_Memory, F1_Biased) |>
...
ggplot(power_grid, aes(x = factor(N_Trials), y = factor(N_Participants), fill = F1_Score)) +
  ...
  scale_fill_viridis_c(option = "mako", direction = -1, limits = c(0, 1), 
       ...
       labs(title = "Model Recovery Power Analysis: Trials × Participants",
       subtitle = paste0("F1 > 80% (white cells) = reliably discriminable design. ",
                         "Memory is harder to detect than Biased — it needs both more trials AND more participants."),
...
### Beyond Binary Recovery: Simulation-Based Calibration Checks for Models
...
```{r ch08_calibration_curves_demo}
...
## Conclusion: Model Comparison as Scientific Reasoning

This chapter developed a complete pipeline for comparing validated cognitive
models:

1. **PSIS-LOO** approximates leave-one-out cross-validation from a single
   fit, with Pareto-$\hat{k}$ flagging both numerical unreliability and
   cognitively interesting observations.
2. **Agent-level K-fold CV** provides an honest test of generalization to
   new participants — answering the question PSIS-LOO cannot.
3. **LOO-PIT + POT-C** checks whether the model's predictive *uncertainty*
   is calibrated, not just its mean predictions.
4. **`loo_compare()`** produces a Z-score-interpretable ranking; below 2
   means inconclusive, not that the method failed.
5. **Bayesian stacking** finds the optimal predictive mixture and provides
   a continuous measure of model evidence.
6. **Model SBC** tests the full comparison pipeline's calibration across
   the prior space.

Two design lessons cut across all of these tools:
- The **reversal design** (validated in Ch. 6, extended in Ch. 7) was
  essential — a constant-bias opponent makes the biased and memory models
  statistically inseparable, and no amount of data fixes a confounded design.
- **All models must pass their individual quality checks before comparison**.
  Comparing a well-fitted model against one with divergences is not a fair
  scientific test.

Chapter 9 introduces a new model type — mixture models — that extends the
toolkit beyond single-process accounts and applies the same PSIS-LOO
machinery to a qualitatively different identifiability problem.
