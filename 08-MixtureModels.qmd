...
# Mixture Models {#sec-mixture-models}
...
> Chs. 1–7 modelled behaviour as arising from a *single* cognitive process
> per agent. This chapter relaxes that assumption: behaviour can be a
> weighted mixture of distinct processes (e.g., deliberate strategy and
> attentional lapses). Mixture models extend the toolkit from Ch. 7
> (hierarchical structures) and Ch. 8 (model comparison via LOO) to this
> new model class. The key new challenge is that discrete latent variables
> — which process generated this trial? — cannot be sampled by HMC and must
> be analytically integrated out before any Stan code is written. The full course roadmap is in Ch. 1, Section "The Course Roadmap".
...
##  Motivation: When are Mixtures Visible?
...
###  The Latent-Variable Augmentation
...
###  Why Stan Requires Marginalisation
...
###  Our Running Model: Bias–Noise Mixture for Binary Choices
...
##  Stan Implementation: Single-Agent Model

###  A Critical Vectorisation Trap
...
###  The Correct Stan Model

::: {.callout-note title="Stan Technique — Marginalizing Discrete Parameters with `log_mix`"}

Stan's HMC sampler requires a **continuous, differentiable** log-posterior. A latent component indicator $z_i \in \{0, 1\}$ (which component generated trial $i$) is **discrete** — HMC cannot sample it directly.

The solution is **analytic marginalization**: instead of sampling $z_i$, we sum its log-probability over both possible values. For a two-component mixture this is exactly what `log_mix` does:

```stan
target += log_mix(pi,          // mixing weight (probability of component 1)
                  lp1,         // log p(obs | component 1)
                  lp2);        // log p(obs | component 2)
```

which computes $\log\!\left[\pi \cdot p_1 + (1-\pi) \cdot p_2\right]$ in a numerically stable way.

**Block placement note.** The per-observation `log_mix` sum lives inside the `model` block as a local computation — it contributes to `target` but `z_i` is never stored. The parameters we do sample (`alpha`, `nu`) are continuous and HMC can take gradients through them. In `generated quantities`, we can *recover* posterior expectations of $z_i$ via a Bayes-rule calculation if needed, but we do not sample it.

**Why the loop is necessary here.** Unlike a pure Bernoulli likelihood, `log_mix` operates on a single observation at a time. There is no vectorized Stan function for mixture likelihoods across observations (because each call is itself a sum of two terms that depend on the observation), so the explicit `for (i in 1:n)` loop is correct and unavoidable for the single-subject model. For the multilevel model we will see how to structure this loop across subjects to enable future parallelization.
:::

```{r ch09_write-single-stan}
single_mixture_stan <- "
data {
  int<lower=1> n;
...
write_stan_file(single_mixture_stan, dir = "stan/", basename = "ch09_mixture_single.stan")
mod_single <- cmdstan_model(
  "stan/ch09_mixture_single.stan"
)
```
...
```{r ch09_load-data}
d    <- read_csv(here("simdata", "ch05_randomnoise.csv"), show_col_types = FALSE)
dd   <- d |> dplyr::filter(true_rate == 0.8, true_noise == 0.1)
data <- list(
  n    = nrow(dd),
  h    = as.integer(dd$choice),
  prior_alpha_mu    = 0,   prior_alpha_sigma = 1,
  prior_nu_mu       = -1,  prior_nu_sigma    = 1,
  run_diagnostics   = 1L
)
...
###  Fitting with Pathfinder Initialisation
...
```{r ch09_fit-single}
single_fit_file <- "simmodels/ch09_single_mixture.RDS"
...
```{r ch09_sbc-run}
sbc_file <- "simmodels/ch09_sbc_results.RDS"
...
###  Calibration: ECDF-Difference Plots
...
```{r ch09_sbc-ecdf-diff, fig.height=4}
...
```{r ch09_sbc-rank-hist, fig.height=4}
...
```{r ch09_sbc-recovery, fig.height=4}
...
##  Identifiability: The Bernoulli Aliasing Problem
...
###  Visualising the Posterior Ridge

```{r ch09_aliasing-manifold}
...
```{r ch09_priorsense-plot, fig.height=5}
...
##  Posterior Predictive Checks and LOO
...
pit_vals <- rstantools::loo_pit(object = yrep_int, y = y, lw = lw)
```
...
write_stan_file(multilevel_mixture_stan,
                dir = "stan/", basename = "ch09_mixture_multilevel.stan")
mod_multi <- cmdstan_model(
  "stan/ch09_mixture_multilevel.stan"
)
```

###  Simulating Multilevel Data

```{r ch09_simulate-multilevel}
set.seed(801)
n_agents <- 30
n_trials <- 120
...
data_multi <- list(
  ...
  agents            = n_agents,
  h                 = h_matrix,
  prior_muA_mu      = 0,    prior_muA_sigma   = 1.0,
  prior_muN_mu      = -1,   prior_muN_sigma   = 0.5,
  prior_tau_sigma   = 0.3,
  prior_lkj_shape   = 2.0,
  run_diagnostics   = 1L
)
...
```{r ch09_fit-multilevel}
multi_fit_file <- "simmodels/ch09_multilevel_mixture.RDS"
...
write_stan_file(biased_stan, dir = "stan/", basename = "ch09_biased_single.stan")
mod_biased <- cmdstan_model(
  "stan/ch09_biased_single.stan"
)
```

```{r ch09_fit-biased}
biased_fit_file <- "simmodels/ch09_biased_single.RDS"
...
**Coming up in Chapter 10:** We leave the matching-pennies domain entirely
and apply the same six-phase workflow to a social learning task — the marble
task — where participants integrate direct evidence about a jar's composition
with social information from another person. This introduces two genuinely
new cognitive models (the Simple and Weighted Bayesian Agents), a
Beta-Binomial likelihood, and the Leave-Future-Out cross-validation technique
needed when trial order creates causal dependencies that standard LOO cannot
handle.
