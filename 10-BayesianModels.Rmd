...
create_diagnostic_plots <- function(fit, model_name) {
  ...
  # Create trace plots
  trace_plot <- mcmc_trace(draws, pars = c("weight_direct", "weight_social"),
                       facet_args = list(ncol = 1)) +
    ggtitle(paste("Trace Plots for", model_name)) +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5, face = "bold"))
  
  # Create rank plots
  rank_plot <- mcmc_rank_hist(draws, pars = c("weight_direct", "weight_social")) +
    ggtitle(paste("Rank Plots for", model_name)) +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5, face = "bold"))
  
  # Return the plots
  return(list(trace_plot = trace_plot, rank_plot = rank_plot))
}

# Generate diagnostic plots for each model
balanced_plots <- create_diagnostic_plots(fit_weighted_balanced, "Balanced Model")
self_focused_plots <- create_diagnostic_plots(fit_weighted_self_focused, "Self-Focused Model")
socially_influenced_plots <- create_diagnostic_plots(fit_weighted_socially_influenced, "Socially-Influenced Model")
...
generate_agent_decisions <- function(weight_direct, weight_social, evidence_df, trials) {
  # Expand dataset to account for multiple trials of the same evidence combination
  ...
  decisions <- pmap_dfr(expanded_data, function(blue1, blue2, total1, total2) {
    ...
    tibble(
      ...
      total2 = total2,
      expected_rate = result$expected_rate,   # Probability the next marble is blue
      choice = ifelse(result$decision == "Blue", 1, 0), # Convert to binary format for Stan
      confidence = result$confidence          # Confidence in decision
    )
  })
...
### Understanding the Results: A Pedagogical Breakdown

Now let's take a deeper look at what these LOO comparisons tell us in cognitive terms:
...
### Conclusion: What Does This Tell Us About Bayesian Cognitive Models?

By comparing our models using LOO-CV, we gain several important insights:

1. **Model Selection Guidance**: LOO provides a principled way to select between competing cognitive models, balancing fit and complexity automatically.

2. **Evidence for Differential Weighting**: In scenarios where agents truly weight evidence sources differently (Self-Focused and Socially-Influenced), the weighted model shows an advantage, suggesting that differential weighting is an important aspect of cognitive processing.

3. **Parsimony Principle**: For the Balanced agent, if the simpler model performs similarly to the complex one, this supports the cognitive principle of parsimony - humans may default to treating evidence sources equally unless there's a reason to weight them differently.

4. **Individual Differences**: The pattern of results across scenarios suggests that individual differences in evidence weighting are meaningful and detectable through model comparison.

In cognitive science terms, our analysis suggests that humans likely do employ differential weighting of evidence sources, and that this weighting varies across individuals. This has implications for understanding decision-making, social influence, and information processing more broadly.
