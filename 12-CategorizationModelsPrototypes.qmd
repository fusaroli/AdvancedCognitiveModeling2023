...
The prototype approach to categorization did not emerge fully formed. It arose as the solution to a specific failure mode of the dominant view — and then itself had to be rescued from static simplicity by importing engineering mathematics.
...
The decisive conceptual move was to ask not *what is stored* but *how the representation is updated*. A static average treats all observed exemplars symmetrarily regardless of when they were encountered. This is inconsistent with the well-documented finding that recent experiences carry more weight than remote ones [@gluck1988conditioning; @estes1994classification]. It is also inconsistent with the ability to track categories that drift over time — the clothing you learn is "fashionable" today may not be fashionable in five years.

The Kalman filter [@kalman1960new], developed originally for aerospace navigation, provided an off-the-shelf solution. It is the **minimum-variance unbiased estimator** for a linear-Gaussian dynamical system: at each step it computes exactly how much weight to place on the new observation versus the accumulated prior, using the relative magnitudes of measurement noise $R$ and process noise $Q$. When $Q = 0$ the prototype is assumed static and learning rate decays monotonically — recovering the classical static average as a special case. When $Q > 0$ the prototype can drift and the steady-state Kalman gain remains positive, allowing perpetual sensitivity to new observations.

Applications of Kalman-style updating to human learning appear across multiple domains: classical conditioning [@dayan2000learning], interval timing, and reward prediction. The prototype-learning context is a natural fit because the Kalman filter formalises the very trade-off prototype theory requires: balance between the accumulated representation (prior) and the informational content of the newest instance (likelihood). The two-parameter family $(r, q)$ instantiates the entire spectrum from pure exemplar-like responsiveness ($r$ small, steep gain) to heavily smoothed prototype abstraction ($r$ large, shallow gain, slow forgetting).

#### The Bayesian Identity

The Kalman filter is not merely an engineering heuristic applied to psychology: it is the exact Bayesian posterior for a Gaussian state-space model. The prototype at trial $t$ is the posterior mean $\mathbb{E}[\mu_t \mid x_{1:t}]$; the covariance $\Sigma_t$ is the posterior variance. Recognising this identity connects the prototype model to the broader programme of **rational analysis** [@anderson1991adaptive] and **Bayesian models of cognition** [@tenenbaum2011grow]: the learner is performing optimal inference about a latent category structure given noisy, sequentially arriving evidence. The parameters $r$ and $q$ then carry clear psychological interpretations — observation noise (how variable are category members around the prototype?) and process noise (how rapidly does the category itself change?) — rather than being purely phenomenological fitting coefficients.
...
As established in the canonical scenario framework introduced in Chapter 11, all three categorization chapters use the same three scenarios in the same order (static → contingent shift → drift), reflecting increasing assumption match between the scenario's data-generating process and the model's structural commitments.
...
if (!is.null(fit_proto_ml)) {
  ...
  diag_tbl_ml <- diagnostic_summary_table(fit_proto_ml, params = pop_vars_ml)
  print(diag_tbl_ml)
...
### Cross-Scenario Population Recovery

Mirroring Chapter 11's three-population analysis, we simulate one population per scenario and verify that the multilevel prototype model recovers population parameters with the same scenario-dependence observed at the single-subject level. The key prediction: `pop_log_r_mean` should be well-recovered in all three scenarios (observation noise directly shapes every per-trial likelihood); `pop_log_q_mean` should be poorly recovered in static data (no drift to track) and cleanly recovered in the smooth-drift scenario (assumption-matched generative process).
...
ggplot(ml_proto_scen_recovery,
  ...
  labs(
    title    = "Multilevel Population Recovery Across Scenarios",
    subtitle = "Points = posterior mean; bars = 90% CI; red dashes = true generating values",
...
* **Comparison with single-subject SBC (§"Simulation-Based Calibration Checks" above).** The single-subject SBC used $S = 500$ iterations of one agent each; the multilevel SBC uses $S = 200$ iterations of 10 agents each. The multilevel SBC is the authoritative check for multi-subject inference: it verifies that partial pooling does not distort individual-level posteriors and that population hyperparameters are jointly calibrated with per-subject parameters.

### Multilevel SBC on Drift Data: The Assumption-Matched Case

The SBC above uses static data, which deliberately exposes the `pop_log_q` calibration failure — the population-level counterpart of the single-subject failure documented in §"Simulation-Based Calibration Checks". We now run a second multilevel SBC using the **smooth-drift** scenario, where the Kalman filter's generative story is literally true of the environment. This is the assumption-matched multilevel check: if the model is correctly specified *and* the task provides drift signal at the individual level, the population-level posterior should be well-calibrated across both parameters.
...
**Reading the drift multilevel SBC.** Under the assumption-matched scenario all six curves should remain inside the simultaneous band and rank histograms should be approximately uniform. Contrast with the static multilevel SBC above: `pop_log_q_mean` and `subj_log_q[1]` should shift from calibration failure (static) to calibration success (drift). `pop_log_q_sd` is the most demanding curve — even on drift data, estimating the *spread* of individual $\log q$ values requires enough between-subject variability to be visible above the within-subject noise. The two multilevel SBC panels together — static as failure case, drift as success case — provide the population-level analogue of the single-subject SBC trilogy from §"Non-Stationary Environments".
...
### The Idea: Rescaling the Feature Space

The conceptually cleanest way to add attention is to **rescale each feature dimension before the model does anything at all**. Define a vector of attention weights $\mathbf{w} = (w_1, w_2, \ldots, w_K)$ with $w_k \geq 0$ and $\sum_k w_k = 1$. Then, before every computation — learning and decision alike — replace the raw stimulus vector $\vec{x}$ with its attention-weighted version:

$$\tilde{x}_k = w_k \cdot x_k$$

The Kalman filter then operates entirely in this **rescaled feature space**. Prototype means, prototype covariances, Kalman gain, and decision probabilities are all computed on $\tilde{\mathbf{x}}$, not on the raw $\mathbf{x}$.

Geometrically, high $w_k$ *magnifies* dimension $k$: small raw differences on that feature become large differences in the rescaled space, making the model more sensitive to variation there. Low $w_k$ *compresses* dimension $k$: large raw differences shrink to nearly nothing, and that feature barely influences the prototype update or the decision.
...
if (regenerate_simulations || !file.exists(attn_sim_file)) {
  ...
  attention_responses <- future_pmap_dfr(
    ...
    left_join(
      attn_param_df |> select(agent_id, condition, w1, w2),
...
It is worth being precise about where `w` and `r` enter the calculation, because the two parameters are sometimes conflated.

**Distance.** After rescaling, the quantity that actually measures how far a stimulus is from a prototype is the Mahalanobis distance in the rescaled space:

$$d_C = (\mathbf{w} \odot \vec{x} - \mathbf{w} \odot \vec{\mu}_C)^T (\Sigma_C + R)^{-1} (\mathbf{w} \odot \vec{x} - \mathbf{w} \odot \vec{\mu}_C)$$

The attention weights $\mathbf{w}$ live entirely inside this distance expression. A feature with high $w_k$ is magnified before the subtraction, so a one-unit raw difference on that dimension contributes more to $d_C$ than a one-unit difference on a low-$w_k$ dimension. Attention shapes the *geometry* of the feature space: it determines which dimensions count as "far apart."

**Similarity.** The similarity of a stimulus to a prototype is the exponential of (minus half) the distance:

$$\text{similarity}_C \propto \exp\!\left(-\tfrac{1}{2}\, d_C\right)$$

This is just the multivariate normal density, which is what `dmvnorm` / `multi_normal_lpdf` computes. The attention weights do not touch this step. Once the distance is computed, the conversion to similarity is fixed in shape.

**The role of $r$.** The observation noise $r$ enters through $(\Sigma_C + R)^{-1}$ inside the distance expression. It controls how quickly similarity falls off with distance — the analogue of the GCM's sensitivity parameter $c$. Large $r$ inflates $R$, which inflates the denominator of the Mahalanobis distance, which shrinks $d_C$ for any fixed raw difference. The result is a flatter bell curve: even stimuli far from the prototype receive non-trivial similarity. Small $r$ does the opposite: the bell curve is sharp and similarity drops steeply with distance.

**Why the two parameters are not redundant.** Multiplying all $w_k$ by a constant $\lambda$ multiplies every element of $d_C$ by $\lambda^2$. Multiplying $r$ by $\lambda^2$ divides the same expression by $\lambda^2$. So if we allowed $\mathbf{w}$ to be an unconstrained positive vector, we could always absorb any rescaling of $\mathbf{w}$ into $r$, and the likelihood would be unchanged — a perfect ridge. The simplex constraint $\sum_k w_k = 1$ breaks this: once the total scale of $\mathbf{w}$ is fixed, only the *relative* weighting across features is free, and that cannot be absorbed into the scalar $r$. The two parameters then play genuinely distinct roles: $\mathbf{w}$ controls the *shape* of the distance metric (which features matter), $r$ controls its *overall scale* (how steeply similarity falls off).

This is the structural reason why Option 2 (rescale features, constrain $\mathbf{w}$ to the simplex) gives a well-identified posterior, while Option 1 (inflate noise by $r / w_k$, no simplex constraint) does not.
...
::: {.callout-warning title="Prepare for a structural shift in Chapter 13"}
Chapter 13 differs from Chapters 11 and 12 in one load-bearing way: **the cognitive theory (a Bayesian particle filter over discrete rule hypotheses) has no Stan-tractable likelihood**. The resampling step is non-differentiable and the latent particle trajectories cannot be marginalised analytically. The chapter therefore introduces two distinct models — Model A, the cognitive theory fitted via Neural Posterior Estimation (NPE); and Model B, a tractable Stan proxy (a marginalised mixture over a fixed candidate rule set). The validation battery applies separately to each. A flat SBC rank histogram for Model B does *not* validate Model A. The gap between the two posteriors on $\varepsilon$ is the chapter's main empirical finding: the structural bias you accept when you replace an intractable cognitive theory with a tractable proxy.

Coming from this chapter, the key conceptual shift is: the Kalman filter gave us a clean two-parameter Stan model where process noise $q$ was both interpretable (drift speed) and identifiable from non-stationary data. The rule particle filter has an analogous mutation parameter $\mu$, but it enters as a general-purpose rejuvenation mechanism rather than as a literal model of how the environment changes — which means identifiability patterns differ, and the assumption-match story is less tight. Chapter 13's scenario analysis (§"Assumption Match Across Scenarios") spells this out.

:::

Chapter 13 also confronts the NPE pipeline that replaces the ABC-based approach of earlier drafts: schedule-fixed simulations, a learned embedding network, and SBC + expected coverage for validation. The cross-architecture comparison in Chapter 15 inherits these NPE networks — each model gets one trained inference network, and the full multi-subject comparison runs in seconds thanks to amortized inference.
