...
> | 6b. SBC (single-subject) | §"Simulation-Based Calibration Checks" |
...
::: {.callout-note title="Scenario order differs from Chapter 11"}
Chapter 11 arranged the three scenarios in order of increasing $\lambda$ preference (static → drift → contingent shift). Here the order is **static → contingent shift → drift**, which reflects increasing *assumption match* between the Kalman filter's Gaussian-drift prior and the data-generating process. The scenarios are the same; the ordering principle is different because the two models have different structural homes for non-stationarity.
:::
