...
> (population and individual calibration, extensive parameter recovery).
...
> | 6a. Parameter recovery (single) | §"Parameter Recovery" |
...
> | 6aML. Individual recovery | §"Individual Parameter Recovery" |
> | 6bML. Extensive recovery | §"Extensive Parameter Recovery for the Multilevel Prototype Model" |
...
if (!is.null(fit_proto_single)) {
  ...
  print(ps)
  priorsense::powerscale_plot_dens(ps, variables = c("log_r", "r_value", "log_q", "q_value"))
}
```
...
```{r ch13_recovery_simulation}
# Run one recovery iteration for a given true r_value, q_value and dataset seed.
# Each dataset gets its own per-agent randomized schedule.
run_single_recovery <- function(true_r_value, true_q_value, iteration,
  ...
  draws_q <- fit$draws("q_value", format = "df")
  tibble(
    ...
    estimated_q_mean = mean(draws_q$q_value),
    est_lower_95_r   = quantile(draws_r$r_value, 0.025),
    est_upper_95_r   = quantile(draws_r$r_value, 0.975),
    ...
    covers_truth_q   = (true_q_value >= quantile(draws_q$q_value, 0.025)) &
                       (true_q_value <= quantile(draws_q$q_value, 0.975)),
    had_divergence   = any(fit$diagnostic_summary(quiet=TRUE)$num_divergent > 0)
  )
}

r_values_to_test <- c(0.5, 1.0, 2.0)
q_values_to_test <- c(0.05, 0.2, 0.5)
n_recovery_iters <- 5   # increase to 20–50 for publication-quality recovery

recovery_file <- here("simdata", "ch13_prototype_recovery_results.rds")
...
recovery_long <- recovery_results |>
  pivot_longer(
    cols      = c(estimated_r_mean, estimated_q_mean),
    names_to  = "variable",
    values_to = "estimated_mean"
  ) |>
  mutate(
    true_value = if_else(variable == "estimated_r_mean", true_r_value, true_q_value),
    variable   = if_else(variable == "estimated_r_mean", "r", "q")
  )

ggplot(recovery_long, aes(x = true_value, y = estimated_mean)) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "grey50") +
  geom_point(aes(color = had_divergence), size = 2, alpha = 0.7) +
  scale_color_manual(values = c("FALSE" = "#0072B2", "TRUE" = "#D55E00"),
                     name = "Had divergence") +
  facet_wrap(~variable, scales = "free") +
  labs(
    title    = "Parameter Recovery: Prototype Model (r and q)",
    subtitle = paste0("Points: posterior mean; n=", n_recovery_iters,
                      " datasets per true value. Dashed line = perfect recovery."),
    x = "True value", y = "Estimated value (posterior mean)"
  )
```

**Discussion of Recovery Results**: The two panels show recovery for $r$ and $q$ separately. Points near the diagonal indicate accurate recovery; divergent iterations are flagged in orange. Under the static Kruschke (1993) paradigm, $q$ and $r$ are partially confounded — both influence the effective likelihood width at decision time. $q$ is best separately identified from non-stationary data where performance oscillates or recovers after a structural change. The recovery plot for $q$ will typically show wider credible intervals and more shrinkage toward the prior mean than the recovery plot for $r$, reflecting this identifiability limitation. **The next subsection turns this observation into a quantitative statement** by sweeping the recovery loop over a grid of trial counts.

### Precision Analysis: How CI Width Scales With N

Recovery at a single trial count tells us the model is identifiable; it does not tell us *how many trials we need* to estimate $r$ and $q$ within a given precision. We can run the recovery loop at multiple dataset sizes and tracking median CI width and empirical coverage as functions of $N$. This is the honest version of "how long should I make my experiment." It is also the cleanest place in this trio of chapters to teach precision analysis, because the two-parameter model and fast simulator make the sweep tractable.
...
```{r ch13_precision_analysis}
precision_file <- here("simdata", "ch13_prototype_precision_results.rds")
if (regenerate_simulations || !file.exists(precision_file)) {
  ...
  precision_results <- future_pmap_dfr(
    list(true_r_value      = precision_plan$true_r_value,
         true_q_value      = precision_plan$true_q_value,
         iteration         = precision_plan$iteration,
         n_blocks_override = precision_plan$n_blocks_grid),
    function(true_r_value, true_q_value, iteration, n_blocks_override) {
      run_single_recovery(true_r_value, true_q_value, iteration,
                          stimulus_info = stimulus_info,
                          n_blocks = n_blocks,
...
if (exists("precision_results")) {
  precision_summary <- precision_results |>
    group_by(true_r_value, true_q_value, n_trials) |>
    summarise(
  ...
  p_width <- ggplot(precision_summary,
    ...
    facet_wrap(~paste0("q = ", true_q_value), ncol = 3) +
    labs(
...
```{r ch13_multilevel_individual_recovery}
if (!is.null(fit_proto_ml)) {
  subj_logr_summary <- fit_proto_ml$summary(variables = "subj_log_r") |>
    mutate(
      agent_id = readr::parse_number(
        stringr::str_extract(variable, "\\[\\d+\\]")
      )
    ) |>
    left_join(multilevel_proto_params |>
                dplyr::select(agent_id, log_r_true, r_true),
              by = "agent_id")

  p_recov_logr <- ggplot(subj_logr_summary,
                         aes(x = log_r_true, y = mean)) +
    geom_point(alpha = 0.7, color = "#009E73") +
    geom_errorbar(aes(ymin = q5, ymax = q95), alpha = 0.3,
                  color = "#009E73", width = 0) +
    geom_abline(intercept = 0, slope = 1,
                color = "red", linetype = "dashed") +
    geom_smooth(method = lm, color = "#0072B2",
                linewidth = 0.7, se = TRUE) +
    labs(
      title    = "Individual Recovery: log(r)",
      subtitle = "Posterior mean ± 90% CI vs. true value",
      x = "True log(r)", y = "Estimated log(r) (posterior mean)"
    )

  subj_r_summary <- fit_proto_ml$summary(variables = "subj_r") |>
    mutate(
      agent_id = readr::parse_number(
        stringr::str_extract(variable, "\\[\\d+\\]")
      )
    ...
    left_join(multilevel_proto_params |>
                dplyr::select(agent_id, r_true),
              by = "agent_id")

  p_recov_r <- ggplot(subj_r_summary,
                      aes(x = r_true, y = mean)) +
    geom_point(alpha = 0.7, color = "#E69F00") +
    geom_errorbar(aes(ymin = q5, ymax = q95), alpha = 0.3,
                  color = "#E69F00", width = 0) +
    geom_abline(intercept = 0, slope = 1,
                color = "red", linetype = "dashed") +
    geom_smooth(method = lm, color = "#0072B2",
                linewidth = 0.7, se = TRUE) +
    labs(
      title    = "Individual Recovery: r (natural scale)",
      subtitle = "Posterior mean ± 90% CI vs. true value",
      x = "True r", y = "Estimated r (posterior mean)"
    )

  cat("=== Individual Parameter Recovery ===\n")
  print(p_recov_logr | p_recov_r)
}
```

**Reading the recovery plots.** Points should cluster around the identity line (red dashed). Shrinkage toward the population mean is expected and desirable — it is the partial-pooling that makes multilevel inference more accurate than independent single-subject fits. Subjects with extreme true values will be pulled toward the group mean; the 90% credible intervals should still contain the truth at the expected rate (verified by the SBC below).

---

## Extensive Parameter Recovery for the Multilevel Prototype Model

```{r ch13_extensive_recovery_ml}
param_grid_ml_proto <- expand_grid(
  pop_log_r_mean = c(log(0.5), log(1.0), log(2.0)),
  pop_log_r_sd   = c(0.2, 0.5)
) |>
  mutate(grid_id = row_number())

n_sims_per_grid_proto  <- 3
n_subjects_recov_proto <- 15

run_proto_ml_recovery_iteration <- function(
    grid_id, pop_log_r_mean, pop_log_r_sd, sim_iteration) {

  cat("Grid", grid_id, "Iter", sim_iteration, "\n")
  mod_local <- cmdstanr::cmdstan_model(here("stan", "ch13_prototype_ml.stan"))

  sim_out <- tryCatch(
    simulate_multilevel_prototype(
      n_subjects     = n_subjects_recov_proto,
      stimulus_info  = stimulus_info,
      n_blocks       = n_blocks,
  ...
  stan_dat <- list(
    ...
    y                         = as.integer(sorted_dat$sim_response),
    cat_one                   = as.integer(sorted_dat$category_feedback),
  ...
  pf_r <- tryCatch(
    mod_local$pathfinder(data = stan_dat, num_paths = 4, refresh = 0,
  ...
  fit_r <- tryCatch(
    mod_local$sample(
      data = stan_dat, init = pf_r,
      seed = grid_id * 100 + sim_iteration,
      chains = 2, parallel_chains = 2,
      iter_warmup = 800, iter_sampling = 1000,
      refresh = 0, adapt_delta = 0.95, max_treedepth = 12
    ),
    error = function(e) { cat("Fit error:", e$message, "\n"); NULL }
  )
  ...
  pop_est <- fit_r$summary(
    variables = c("pop_log_r_mean", "pop_log_r_sd"),
    "mean"
  ) |>
    mutate(
      true_value = case_when(
        variable == "pop_log_r_mean" ~ pop_log_r_mean,
        variable == "pop_log_r_sd"   ~ pop_log_r_sd,
        TRUE                         ~ NA_real_
      ),
      grid_id        = grid_id,
...
recovery_plan_proto <- param_grid_ml_proto |>
  crossing(sim_iteration = seq_len(n_sims_per_grid_proto))

recovery_file_proto <- here("simdata", "ch13_proto_ml_recovery_raw.csv")

if (regenerate_simulations || !file.exists(recovery_file_proto)) {
  plan(multisession, workers = max(1, availableCores() %/% 2))
  proto_recovery_raw <- future_pmap_dfr(
    list(
      grid_id        = recovery_plan_proto$grid_id,
      pop_log_r_mean = recovery_plan_proto$pop_log_r_mean,
      pop_log_r_sd   = recovery_plan_proto$pop_log_r_sd,
      sim_iteration  = recovery_plan_proto$sim_iteration
    ),
    run_proto_ml_recovery_iteration,
    .options = furrr_options(seed = TRUE, scheduling = 1)
  )
  plan(sequential)
  write_csv(proto_recovery_raw, recovery_file_proto)
} else {
  proto_recovery_raw <- read_csv(recovery_file_proto, show_col_types = FALSE)
}

proto_recovery_labeled <- proto_recovery_raw |>
  left_join(param_grid_ml_proto |>
              dplyr::select(grid_id, pop_log_r_mean, pop_log_r_sd),
            by = "grid_id")

ggplot(proto_recovery_labeled |> filter(!is.na(true_value)),
  ...
  facet_wrap(~variable, scales = "free") +
  labs(
    title    = "Population Parameter Recovery: Multilevel Prototype Model",
    subtitle = sprintf(
      "%d grid cells × %d simulations × %d subjects each",
      nrow(param_grid_ml_proto), n_sims_per_grid_proto, n_subjects_recov_proto
    ),
