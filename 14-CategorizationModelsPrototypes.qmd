...
* **Divergence profile.** The scatter of divergences against the true `pop_log_r_sd` and `pop_log_q_sd` reveals whether failures concentrate in the small-SD region — the characteristic signature of the Neal's funnel geometry. The non-centred parameterisation handles moderate funnels well; if divergences are frequent even at larger SD values, increase `adapt_delta` or switch the SD hyperparameter to a half-Normal prior with a tighter scale.
...
**Reading the drift multilevel SBC.** Under the assumption-matched scenario all six curves should remain inside the simultaneous band and rank histograms should be approximately uniform. Contrast with the static multilevel SBC above: `pop_log_q_mean` and `subj_log_q[1]` should shift from calibration failure (static) to calibration success (drift). `pop_log_q_sd` is the most demanding curve — even on drift data, estimating the *spread* of individual $\log q$ values requires enough between-subject variability to be visible above the within-subject noise.

The recovery scatter plots give an orthogonal read: points for `pop_log_q_mean` and `subj_log_q[1]` that were diffuse on static data should tighten around the identity line here, while `pop_log_r_mean` and `subj_log_r[1]` should remain well-recovered across both scenarios. Curvature in `pop_log_r_sd` or `pop_log_q_sd` at the low end would indicate that the prior on the SD hyperparameter is pulling estimates away from near-zero true values — a common partial-pooling artefact when between-subject variability is genuinely small.

The divergence profile under drift data should be compared directly to the static profile. Any residual divergences are expected to concentrate at small `pop_log_r_sd` and `pop_log_q_sd` (the Neal's funnel region), not in the interior of parameter space. If the drift scenario produces *more* divergences than the static scenario, the larger effective posterior curvature introduced by the drift updates may be straining the NCP geometry; increasing `adapt_delta` to 0.99 or adding a heavier-tailed prior on the SD hyperparameters are the appropriate remedies.

The two multilevel SBC panels together — static as failure case, drift as success case — provide the population-level analogue of the single-subject SBC trilogy from §"Non-Stationary Environments".
