...
> Chapter 12 built the GCM — an exemplar model that stores every encountered
...
For continuous outcomes the preferred calibration check is LOO-PIT. For binary outcomes the equivalent is the *randomised* LOO-PIT (Säilynoja et al. 2022), which avoids the degenerate two-bin distribution that plain LOO-PIT produces on Bernoulli data. Chapter 12 implements this for the GCM; we apply the same construction here.
...
As established in the canonical scenario framework introduced in Chapter 12, all three categorization chapters use the same three scenarios in the same order (static → contingent shift → drift), reflecting increasing assumption match between the scenario's data-generating process and the model's structural commitments.
...
3. Looping the Kalman filter (with prediction step) over subjects in `transformed parameters`, exactly as the GCM does in Chapter 12.
...
A note of caution about ELPD comparisons in Chapter 15: now that LFO-CV is implemented for both the GCM (Chapter 12) and the Kalman prototype (this chapter), the comparison between them is on equal predictive footing. The rule mixture model in Chapter 14 is conditionally exchangeable and uses standard PSIS-LOO, which is technically valid for *that* model — but the comparison across architectures is still a comparison of fundamentally different generative processes, and "ELPD is higher" should not be read as "this is the right cognitive theory." See Chapter 14 for the deeper version of this caveat (and the ABC-based alternative comparison framework that resolves it more cleanly).
