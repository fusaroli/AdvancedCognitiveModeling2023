...
Sections marked **🔄 v2** contain the substantive textual changes. Sections
not so marked are unchanged from v1; a number of them describe inline
`reticulate`-based code that is preserved for pedagogical contrast but is
NOT executed under this revision.
:::
...
python_available <- FALSE  # v2: do NOT call sbi inline.
...
**Step 3 — Train a conditional density estimator $q_\phi(\theta \mid y)$.** This is the neural network. Its job is to take a response sequence $y$ and output a probability distribution over $\theta$ — not just a point estimate, but a full distribution. The training objective is derived from a single Kullback–Leibler statement [@radev2020amortized]: we want the approximate posterior $q_\phi(\theta \mid f_\eta(y))$ — where $f_\eta$ is the summary network and $\phi = (\psi, \eta)$ collects the inference- and summary-network weights — to be as close as possible to the true posterior $p(\theta \mid y)$,
$$\min_{\phi} \; \mathrm{KL}\!\left(p(\theta \mid y) \,\|\, q_\phi(\theta \mid f_\eta(y))\right),$$
averaged over the prior predictive $p(y) = \int p(y \mid \theta)\, p(\theta)\, d\theta$. Because we have direct access to the joint $p(\theta, y)$ through the simulator, the KL minimisation is equivalent to a maximum-likelihood criterion that we can approximate by Monte Carlo over the simulated training set:
$$\min_{\phi} \; \mathbb{E}_{p(\theta, y)}\!\left[-\log q_\phi(\theta \mid f_\eta(y))\right] \;\approx\; -\frac{1}{N}\sum_{i=1}^{N} \log q_\phi\!\left(\theta^{(i)} \mid f_\eta(y^{(i)})\right).$$
The first form makes the target explicit (the true posterior); the second is what we actually optimise. In words: the network is pushed to assign high probability to the parameter that actually generated each simulated dataset, and after enough $(\theta, y)$ pairs it learns to read off the correct posterior from any observation. Under sufficient capacity and asymptotic convergence, the summary network outputs sufficient statistics and the inference network samples from the true posterior [@radev2020amortized; @radev2023bayesflow].
...
### Setting Up the Python Environment 🔄 v2
...
### Step 1 + 2: The Prior and the Simulator 🔄 v2
...
> **🔄 v2 — chunk removed.** The corresponding inline `reticulate`/`sbi` code is no longer needed: simulation, training, and posterior sampling are performed by `scripts/ucloud_npe/amortized_particle_filter.py`. The diagnostic block above (`ch15_v2_load_bayesflow`) loads the resulting artifacts.

### Step 2: Generating the Training Dataset 🔄 v2
...
### NPE Prior Predictive Check 🔄 v2

The prior predictive — what response sequences the prior produces before any inference — is now realised by the `train_data` block saved by the standalone script. To inspect it visually, post-process the script's training simulations into cumulative-accuracy curves and save them as `prior_predictive.png` next to the standard diagnostics. (The script does not currently emit this figure; adding it is a small matplotlib block at the end.) The diagnostic block below auto-includes any image present in `simdata/ch15_v2_bayesflow/` whose name starts with `prior_predictive`.

### Step 3: Training the BayesFlow Inference Network 🔄 v2
...
### Step 4: Performing Inference on the Observed Data 🔄 v2
...
For artifact-driven analysis in this chapter, the standalone script writes posterior samples for the held-out demo subject to `posterior_samples.npy`; the v2 chunk below loads them. To run inference on a new subject from R, call the script with that subject's response sequence (a small CLI argument is the cleanest extension; not implemented here).

**What to look for.** A well-calibrated NPE posterior for $\varepsilon$ should be centred near the true generating value. If the posterior is much wider than the prior, the data are uninformative; if sharply narrower, the response sequence contains strong information. The posterior for $\mu$ is expected to be wider on static data because a static task cannot distinguish a low-mutation learner from a zero-mutation learner. The posterior for $N_\text{particles}$ should now (under the v2 prior) be interpretable on a cognitive scale.

### v2: Loading and displaying BayesFlow artifacts

```{r ch15_v2_load_bayesflow, eval=npe_artifacts_available}
# Load artifacts produced by scripts/ucloud_npe/amortized_particle_filter.py
metrics_path <- file.path(npe_artifacts_dir, "metrics.csv")
history_path <- file.path(npe_artifacts_dir, "history.json")

bf_metrics <- read_csv(metrics_path, show_col_types = FALSE)
bf_metrics |> knitr::kable(caption = "BayesFlow in-silico diagnostics on 300 held-out simulations.")

# Show the standard 5 diagnostic figures from the run
diagnostic_figs <- c(
  "loss"               = "loss.png",
...
for (nm in names(diagnostic_figs)) {
  p <- file.path(npe_artifacts_dir, diagnostic_figs[[nm]])
  if (file.exists(p)) knitr::include_graphics(p)
}

# Quick training-history sanity check
hist <- jsonlite::fromJSON(history_path)
cat("Final loss     :", tail(hist$loss, 1), "\n")
cat("Final val_loss :", if (!is.null(hist$val_loss)) tail(hist$val_loss, 1) else NA, "\n")
```

If `npe_artifacts_available` is `FALSE` (the standalone script has not been run for the `static` scenario, or the artifacts have not been copied into `simdata/ch15_v2_bayesflow/static/`), this chunk is silently skipped and the chapter renders without the diagnostic block.

```{r ch15_v2_prior_predictive_static, eval=npe_artifacts_available}
prior_pp <- file.path(npe_artifacts_dir, "prior_predictive.png")
if (file.exists(prior_pp)) knitr::include_graphics(prior_pp)
```
...
v2: the standalone script computes the PPC by looping 50 posterior draws back through the same `particle_filter()` simulator used for training (per the amortized-workflow skill's "reuse the simulator, never re-implement" rule), and saves `ppc_overlay.png` under the scenario directory. The chunk below embeds it.

```{r ch15_v2_ppc_overlay_static, eval=npe_artifacts_available}
ppc_path <- file.path(npe_artifacts_dir, "ppc_overlay.png")
if (file.exists(ppc_path)) knitr::include_graphics(ppc_path)
```
...
> **🔄 v2 — hand-rolled SBC + coverage removed.** The amortized-workflow skill bans hand-rolled calibration and coverage code. v2 uses `workflow.compute_default_diagnostics(test_data=...)` and `workflow.plot_default_diagnostics(test_data=...)`, which produce the standard `calibration_ecdf.png` and `coverage.png` figures plus a `metrics.csv` summary. These are loaded and displayed by the `ch15_v2_load_bayesflow` chunk above.
...
> **🔄 v2 — hand-rolled contraction chunk removed.** Posterior contraction is included in the standard BayesFlow diagnostic outputs as `z_score_contraction.png` (rendered above) and as a column in `metrics.csv`.
...
> **🔄 v2 — gap-across-seeds chunk removed.** The qualitative claim (the NPE-vs-HMC gap tracks rule-set mismatch) is unchanged. To reproduce it in v2, extend the standalone script to (i) simulate datasets under multiple schedule seeds, (ii) call `workflow.sample` on each, (iii) write a `gap_results.csv` with columns `seed, npe_mean_eps, stan_mean_eps, gap`. A small R chunk can then load and plot it.
...
# Pseudocode (Python side, in the standalone script):
#   for x_s in subject_response_tensors:                  # shape (1, T, 4)
#       samples = workflow.sample(conditions={"trial_series": x_s},
#                                 num_samples=2000)
#       # samples["logit_eps"], samples["log_n_particles"], samples["logit_mu"]
# Total time: ~N * 0.05 seconds on a GPU node, ~N * 0.2 s on CPU.
```
...
### NPE Across the Three Scenarios 🔄 v2
...
```{r ch15_v2_scenarios_diagnostics, results="asis"}
ready <- vapply(NPE_SCENARIOS, npe_scenario_ready, logical(1))
if (any(ready)) {
  for (sc in NPE_SCENARIOS[ready]) {
    cat(sprintf("\n#### Scenario: `%s`\n\n", sc))
    d <- npe_scenario_dir(sc)

    # Numerical metrics
    m <- read_csv(file.path(d, "metrics.csv"), show_col_types = FALSE)
    print(knitr::kable(m, caption = sprintf(
      "BayesFlow in-silico diagnostics on 300 held-out simulations (%s).", sc)))

    # Standard 5 default-diagnostic figures + PPC + prior predictive
    figs <- c(
      "loss.png", "recovery.png", "calibration_ecdf.png",
      "coverage.png", "z_score_contraction.png",
      "prior_predictive.png", "ppc_overlay.png"
    )
    for (f in figs) {
      p <- file.path(d, f)
    ...
    # Final-loss summary from history
    h <- jsonlite::fromJSON(file.path(d, "history.json"))
    cat(sprintf("\n*Final loss = %.3f; final val_loss = %s.*\n",
                tail(h$loss, 1),
                if (!is.null(h$val_loss)) sprintf("%.3f", tail(h$val_loss, 1)) else "NA"))
  }
} else {
  cat("\n*No scenario artifacts found under `", npe_artifacts_root, "`. ",
      "Run the standalone script for one or more scenarios to populate this section.*\n",
      sep = "")
}
```
