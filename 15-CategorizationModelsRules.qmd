...
# ── NPE artifacts produced by the standalone Python script ───────────────────
# All NPE-side computation (training, sampling, calibration, recovery) is
# performed by scripts/ucloud_npe/amortized_particle_filter.py. The script
# writes its outputs to simdata/ch15_v2_bayesflow/<scenario>/ and the chunks
# below load and display them. This means the chapter renders cleanly even
# when no Python/sbi/BayesFlow stack is available locally — the trained
# network and its diagnostics are stored as plain files (PNG, CSV, JSON).
npe_artifacts_root <- here("simdata", "ch15_v2_bayesflow")

npe_scenarios <- c("static", "contingent_shift", "drift")

npe_artifacts_available_for <- function(scenario) {
  d <- file.path(npe_artifacts_root, scenario)
  dir.exists(d) &&
    file.exists(file.path(d, "metrics.csv")) &&
    file.exists(file.path(d, "posterior_eps.csv"))
}

npe_artifacts_dir <- file.path(npe_artifacts_root, "static")
npe_artifacts_available <- npe_artifacts_available_for("static")
...
This chapter closes the exemplar–prototype–rules trilogy. The previous two chapters built similarity-based accounts of categorization — one storing raw instances (GCM), the other maintaining a running Gaussian mean (Kalman prototype). Both admitted differentiable Stan models and followed the same six-phase validation pipeline directly. This chapter introduces a qualitatively different cognitive architecture — hypothesis search over a structured rule space — that breaks differentiability and forces a fundamental methodological choice. Understanding that choice, and its cost, is the chapter's central contribution. The validation structure (simulator → prior predictive → Stan/NPE fit → diagnostic battery → non-stationarity extension) is the same as in Chapters 11 and 12; the dual-model complication is what distinguishes this chapter from its predecessors.
...
::: {.callout-note title="Grammar portability: Chapter 16 (Alien Game)"}
Chapter 16 applies the same grammar and particle filter to 5 binary features. Only four substitutions are required — `n_features` ($2 \to 5$), `feature_range` (continuous extents → `c(0,1)` per feature), threshold sampling (continuous → fixed at 0.5), and `max_dims` ($2 \to 3$). The nested form $A\ L_{\text{outer}}\ (B\ L_{\text{inner}}\ C)$ is retained because the Alien Game grammar `A AND (B OR C)` is exactly that shape. The trial loop, weight update, resampling, and mutation kernel are unchanged. The closing callout "What Carries to Chapter 16" summarises the full correspondence.
:::
...
::: {.callout-tip title="Beyond selection: extensions to the particle filter"}
The basic filter is primarily a **rule selection** mechanism: it identifies the most promising rules from its initial random pool, but the resampling step cannot invent rules that were never sampled. The `mutate_particles()` function (§Step 4) already addresses degeneracy via prior-sampling regeneration. More cognitively motivated extensions include:

1. **Targeted mutation**: adjust thresholds or flip operators only on dimensions where the current rule fails, rather than discarding the rule entirely (see §"Targeted Mutation: Revision Rather Than Replacement").
2. **Recombination/crossover**: combine conditions from two high-weight "parent" rules into a "child" rule, synthesising features discovered independently.
3. **Targeted generation**: generate new rules focused on stimuli consistently misclassified by the current particle set.
4. **Complexity adjustment**: probabilistically grow a 1D rule into a 2D rule or prune a redundant condition.

We do not implement (2–4) here, but they are natural extensions. Any of them makes Model A's intractability *worse* from a Stan standpoint — each adds further stochastic discrete jumps. NPE handles all variants transparently: the inference network only sees $(\theta, y)$ pairs and does not care how the simulator produces them.
:::
...
Before fitting Model B we verify that the prior on `logit_error_prob` generates only scientifically plausible categorisation curves. There is a structural subtlety: **Model B has no forward simulator with learning dynamics.** Its true prior predictive distribution consists of flat Bernoulli curves at $1 - \varepsilon$ — not the rising learning curves observed in human data.

What we show instead — following the convention established in Chapters 11 and 12 — is to draw $\varepsilon$ from the prior and run **Model A's simulator** to generate the prior predictive learning curves. This is a *cross-model* prior predictive: it checks whether Model B's $\varepsilon$ prior is plausible as a prior for the cognitive theory, not whether it is plausible for the proxy. The distinction matters and is flagged explicitly in the plot subtitle.
...
ggplot(ppc_summary, aes(x = trial)) +
  ...
  labs(
    title    = "Prior Predictive Curves — Model A Simulator, Model B ε Prior",
    subtitle = "Cross-model check: ε drawn from Model B's prior, curves generated by Model A.\nModel B's own forward sim would be flat at 1 − ε (not shown).",
    x = "Trial", y = "Cumulative Accuracy"
  )
```

> **Cross-model prior predictive.** The plot uses $\varepsilon$ values sampled from Model B's prior to drive Model A's simulator, producing rising learning curves consistent with the cognitive theory. Model B's own prior predictive would consist of flat curves at $1 - \varepsilon$ — technically valid, but uninformative about whether the prior is cognitively reasonable. We display the cross-model version for pedagogical content and flag it explicitly. The strict within-model check for Model B can be computed by drawing $\varepsilon_s$, drawing a rule index $r_s$ uniformly, and generating Bernoulli responses; the result is flat and unilluminating.
...
Chapters 11 and 12 each ran a single SBC block: data from the model, fitted by the model. This chapter requires two distinct calibration checks because there are two distinct models. **That is not a complication — it is the honest accounting that the Model A / Model B distinction demands.**

Earlier drafts ran one block under the header "Simulation-Based Calibration (SBC)" that generated data with the particle filter (Model A) and fitted Model B. **That is not SBC.** Talts–Vehtari SBC requires the data to be drawn from the *same* generative model that is then fitted, so that the joint $(\theta, y)$ distribution of the inference model is what gets calibrated. When the generative and inferential models differ, what you are running is closer to a robustness check than a calibration check, and labelling it SBC is misleading.

We therefore run two correctly-labelled checks: (1) a **Cross-Model Calibration Check** (data from Model A, fitted by Model B) and (2) a **Real SBC for Model B** (data from Model B itself, fitted by Model B). The first is a robustness check; the second is the actual Talts–Vehtari calibration test. The NPE pipeline for Model A has its own SBC block in §"NPE Validation: SBC + Expected Coverage."
...
The real SBC simulations already produced hundreds of (true $\varepsilon$, fitted posterior) pairs — exactly what parameter recovery needs. We extract the posterior summaries stored in `real_sbc_results$stats` and back-transform from logit to probability scale; no additional simulations are required.

```{r ch15_parameter_recovery}
if (run_intensive_checks && exists("real_sbc_results")) {
  recovery_from_sbc <- real_sbc_results$stats |>
    filter(variable == "logit_error_prob") |>
    mutate(
      true_error_prob = plogis(true_value),
      est_mean        = plogis(mean),
      est_lo          = plogis(q5),
      est_hi          = plogis(q95)
    )

  ggplot(recovery_from_sbc, aes(x = true_error_prob, y = est_mean)) +
    geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "grey50") +
    geom_errorbar(aes(ymin = est_lo, ymax = est_hi), width = 0.003, alpha = 0.4) +
    geom_point(size = 1.5, alpha = 0.6) +
    labs(
      title    = "Parameter Recovery: Model B (error_prob)",
      subtitle = paste0("Each point: one real SBC simulation (n = ", n_real_sbc_iters, ").\n",
                        "Posterior mean ± 90% CrI; data and model both from Model B's generator."),
      x = "True error_prob", y = "Estimated error_prob (posterior mean)"
    )
}
```

**Discussion of Recovery Results.** Because both the data generator and the inference model are Model B, any bias here is a property of the Stan implementation or the prior, not a cross-model mismatch. Points scattered symmetrically around the diagonal with narrow CrIs indicate that HMC recovers $\varepsilon$ without systematic distortion across the full prior range.
...
::: {.callout-note title="Figures in this section"}
All diagnostic plots in this section (training loss curves, prior predictive ribbons, posterior predictive overlays, SBC rank histograms, coverage curves, contraction scatter, and recovery scatter) are built as inline ggplot2 panels, consistent with the rest of the book. The standalone Python script `scripts/ucloud_npe/amortized_particle_filter.py` trains the NPE network and exports raw numerical summaries as CSV files under `simdata/ch15_v2_bayesflow/<scenario>/`; the R chunks below read those CSVs and render the figures using `theme_bw()` and the book's standard colour palette.
:::
...
```{r ch15_npe_simulator, eval=python_available}
...
if (file.exists(pp_curves_file)) {
  ...
  ggplot(pp_summary, aes(x = trial)) +
    ...
    scale_y_continuous(limits = c(0, 1)) +
    labs(
      title    = "NPE Prior Predictive Check (Model A)",
      subtitle = paste0("Median + 50%/90% intervals across ",
                        nrow(pp_summary), "-trial schedule; 500 parameter draws from the NPE prior"),
      x = "Trial", y = "Cumulative accuracy"
    ) +
    theme_bw()
} else {
  cat("> `prior_predictive_curves.csv` not found. Re-run `amortized_particle_filter.py`",
...
```{r ch15_npe_train, eval=python_available}
...
In v2, the chapter does not call the network from R. The standalone script samples the NPE posterior for one held-out demo subject (the first row of the test set, drawn from the prior) and writes the back-transformed draws to `posterior_demo.csv` (`eps`, `n_particles`, `mu`). The chunk below loads those draws and overlays them on the corresponding prior. If the script also wrote `demo_truth.csv` (one row with the demo subject's true `eps`, `n_particles`, `mu`), a reference line is drawn; otherwise the truth line is omitted with a note.

```{r ch15_npe_inference, eval=npe_artifacts_available}
demo_csv  <- file.path(npe_artifacts_dir, "posterior_demo.csv")
truth_csv <- file.path(npe_artifacts_dir, "demo_truth.csv")

npe_demo <- readr::read_csv(demo_csv, show_col_types = FALSE)
npe_eps    <- npe_demo$eps
npe_n_part <- npe_demo$n_particles
npe_mu     <- npe_demo$mu

if (file.exists(truth_csv)) {
  demo_truth <- readr::read_csv(truth_csv, show_col_types = FALSE)
  truth_eps    <- demo_truth$eps[1]
  truth_n_part <- demo_truth$n_particles[1]
  truth_mu     <- demo_truth$mu[1]
} else {
  truth_eps <- truth_n_part <- truth_mu <- NA_real_
}

cat("NPE posterior for ε (demo subject):           mean =", round(mean(npe_eps), 4),
    "  95% CI = [", round(quantile(npe_eps, 0.025), 4), ",",
    round(quantile(npe_eps, 0.975), 4), "]\n")
cat("NPE posterior for N_particles (demo subject): mean =", round(mean(npe_n_part), 1),
    "  95% CI = [", round(quantile(npe_n_part, 0.025), 1), ",",
    round(quantile(npe_n_part, 0.975), 1), "]\n")
cat("NPE posterior for μ (demo subject):           mean =", round(mean(npe_mu), 4),
    "  95% CI = [", round(quantile(npe_mu, 0.025), 4), ",",
    round(quantile(npe_mu, 0.975), 4), "]\n")
if (!is.na(truth_eps)) {
  cat(sprintf("Truth: ε=%.3f  N=%.0f  μ=%.3f\n", truth_eps, truth_n_part, truth_mu))
} else {
  cat("(demo_truth.csv not present — re-run amortized_particle_filter.py to emit it)\n")
}

# Prior overlays on the natural scale, matched to the priors used by the script.
n_prior_draws  <- length(npe_eps)
prior_eps_nat  <- plogis(rnorm(n_prior_draws, LOGIT_ERROR_PRIOR_MEAN, LOGIT_ERROR_PRIOR_SD))
prior_mu_nat   <- plogis(rnorm(n_prior_draws, LOGIT_MU_PRIOR_MEAN,    LOGIT_MU_PRIOR_SD))
prior_n_nat    <- exp(runif(n_prior_draws, log(20), log(300)))

p_eps <- ggplot() +
  geom_density(aes(x = prior_eps_nat), fill = "grey70", alpha = 0.5, linewidth = 0.4) +
  geom_histogram(aes(x = npe_eps, y = after_stat(density)),
                 fill = "#009E73", alpha = 0.7, bins = 40) +
  { if (!is.na(truth_eps))
      geom_vline(xintercept = truth_eps, color = "red", linetype = "dashed")
    else NULL } +
  labs(title = "NPE Posterior for ε (Model A, demo subject)",
       subtitle = "Grey = prior; red dashed = demo subject's true ε (if available)",
       x = "ε", y = "Density")

p_npart <- ggplot() +
  geom_density(aes(x = prior_n_nat), fill = "grey70", alpha = 0.5, linewidth = 0.4) +
  geom_histogram(aes(x = npe_n_part, y = after_stat(density)),
                 fill = "#0072B2", alpha = 0.7, bins = 40) +
  { if (!is.na(truth_n_part))
      geom_vline(xintercept = truth_n_part, color = "red", linetype = "dashed")
    else NULL } +
  labs(title = "NPE Posterior for N_particles (Model A, demo subject)",
       subtitle = "Grey = prior over [20, 300]; red dashed = demo subject's true N (if available)",
       x = "N_particles", y = "Density")

p_mu <- ggplot() +
  geom_density(aes(x = prior_mu_nat), fill = "grey70", alpha = 0.5, linewidth = 0.4) +
  geom_histogram(aes(x = npe_mu, y = after_stat(density)),
                 fill = "#CC79A7", alpha = 0.7, bins = 40) +
  { if (!is.na(truth_mu))
      geom_vline(xintercept = truth_mu, color = "red", linetype = "dashed")
    else NULL } +
  labs(title = "NPE Posterior for μ (Model A, demo subject)",
       subtitle = "Grey = prior. On static data the posterior is expected to remain prior-like.",
       x = "μ", y = "Density")

print(p_eps / p_npart / p_mu)
```

```{r ch15_npe_inference_fallback, eval=!npe_artifacts_available, echo=FALSE, results='asis'}
cat("> **NPE artifacts not found** at `simdata/ch15_v2_bayesflow/static/`.",
    "Run `scripts/ucloud_npe/amortized_particle_filter.py --scenario static`",
    "and copy its output directory under that path to populate this section.\n")
```

**Reading the NPE posterior.** Three things to check: (1) the posterior for $\varepsilon$ should be visibly narrower than the grey prior and centred near the demo subject's true $\varepsilon$ (red dashed); (2) $N_{\text{particles}}$ on a single short response sequence is only loosely identified — modest contraction is the expected outcome, not a sharp peak; (3) the posterior for $\mu$ is **expected to be largely prior-like on static data** — the Kruschke task provides no signal about mutation rate because the correct rule does not change. The (near-)overlap of the $\mu$ posterior with the grey prior is the right result, not a failure. A narrow $\mu$ posterior on non-stationary data would be the substantive finding (we test that prediction in a later subsection).
...
In v2 the PPC is computed by the standalone script: 50 posterior draws are looped back through the *same* `particle_filter()` simulator the network was trained on, and the resulting cumulative-accuracy ribbon is overlaid on the demo subject's observed curve. The figure is saved as `ppc_overlay.png` and embedded below.
...
if (file.exists(ppc_ov_file)) {
  ...
  ggplot(ppc_ov, aes(x = trial)) +
    ...
    geom_line(aes(y = q50), color = "#006D4F", linewidth = 1,
              linetype = "solid") +
    geom_line(aes(y = observed), color = "black", linewidth = 1.2) +
    geom_hline(yintercept = 0.5, linetype = "dashed", color = "grey40") +
    scale_y_continuous(limits = c(0, 1)) +
    annotate("text", x = max(ppc_ov$trial) * 0.6, y = 0.12,
             label = "Black line = observed demo subject", hjust = 0, size = 3.5) +
    labs(
      title    = "NPE Posterior Predictive Check (Model A, demo subject)",
      subtitle = "Green ribbon = 90%/50% PPC interval from 50 posterior draws through the particle filter",
      x = "Trial", y = "Cumulative accuracy"
    ) +
    theme_bw()
} else {
  cat("> `ppc_overlay_curves.csv` not found. Re-run `amortized_particle_filter.py`",
      "with the updated script to generate it.\n")
}
```

The observed curve (black) should lie inside the 90% ribbon for almost all trials; isolated escapes are tolerable, sustained departures are not. Unlike the cross-model PPC shown for Model B (which used Model A's simulator with Model B's prior), this check uses the actual NPE posterior on the actual NPE simulator — it is the within-model PPC the Model B section could not produce.

### NPE Training Loss

Before examining calibration, we check that the network actually trained. The loss curves below show the negative log-probability loss on the training and held-out validation sets across epochs. A well-trained network shows both curves decreasing and converging; divergence between train and val indicates overfitting.
...
if (file.exists(history_file)) {
  ...
  ggplot(loss_df, aes(x = epoch, y = loss, color = split, linetype = split)) +
    ...
    labs(
      title    = "NPE Training Loss (Model A)",
      subtitle = "Negative log-probability per epoch; vertical drop = early stopping restoring best weights",
      x = "Epoch", y = "Negative log-probability loss",
      color = NULL, linetype = NULL
    ) +
    theme_bw() +
    theme(legend.position = "top")
} else {
  cat("> `history.json` not found. Re-run `amortized_particle_filter.py`.\n")
}
```

### NPE Validation: Calibration and Expected Coverage (Phase 6)

Calibration checks for NPE follow the same Talts–Vehtari logic as Model B's SBC, but benefit from amortisation: 300 checks require only 300 forward passes through the trained network, completing in seconds. The updated script exports two CSVs — `sbc_ranks.csv` (rank of the true parameter in 99 posterior draws, for each test case) and `coverage_data.csv` (empirical coverage at 19 nominal levels) — alongside the summary `metrics.csv`.

```{r ch15_npe_metrics_table, eval=npe_artifacts_available}
metrics_path <- file.path(npe_artifacts_dir, "metrics.csv")
if (file.exists(metrics_path)) {
  readr::read_csv(metrics_path, show_col_types = FALSE) |>
    knitr::kable(
      caption = "NPE in-silico diagnostics on 300 held-out simulations (static scenario).",
      digits  = 3
    )
}
```
...
if (file.exists(sbc_file)) {
  ...
  n_ranks <- nrow(sbc_df) / n_distinct(sbc_df$parameter)
  n_bins  <- 10L
  expected_count <- n_ranks / n_bins

  ggplot(sbc_df, aes(x = rank)) +
    geom_histogram(bins = n_bins, fill = "#0072B2", color = "white", alpha = 0.8) +
    geom_hline(yintercept = expected_count, color = "red",
               linetype = "dashed", linewidth = 0.7) +
    facet_wrap(~parameter, nrow = 1) +
    labs(
      title    = "NPE Calibration: SBC Rank Histograms (Model A, static scenario)",
      subtitle = "Red dashed = expected count under uniform calibration. Flat histogram = well-calibrated.",
      x = "Rank of true value in 99 posterior draws", y = "Count"
    ) +
    theme_bw()
} else {
  cat("> `sbc_ranks.csv` not found. Re-run the updated `amortized_particle_filter.py`.\n")
}
```
...
if (file.exists(cov_file)) {
  ...
  ggplot(cov_df, aes(x = nominal, y = empirical)) +
    ...
    labs(
      ...
      subtitle = "Points on the diagonal = nominal credible intervals contain truth at the stated rate.",
      x = "Nominal coverage", y = "Empirical coverage"
    ) +
    theme_bw()
} else {
  cat("> `coverage_data.csv` not found. Re-run the updated `amortized_particle_filter.py`.\n")
}
```

**Reading calibration.** The rank histogram for $\varepsilon$ should be approximately flat — the posterior is genuinely informative and well-calibrated. The histogram for $\mu$ on static data requires care: a flat histogram here means "the posterior is prior-like for the right reason" — the data contain no mutation signal, so the posterior correctly remains uncertain. A systematic departure from flat (e.g., U-shaped for $\mu$) would indicate false confidence. The coverage plot on the diagonal confirms the same: nominal 90% intervals should contain the truth in ~90% of calibration draws.

### Posterior Contraction and Parameter Recovery

Calibration tells us the posterior is *honest*; contraction tells us how much the data actually *inform* it. For each calibration draw we compare the posterior SD to the prior SD: contraction near 0 means the posterior is as wide as the prior; near 1 means the posterior has collapsed to the truth.
...
if (file.exists(cont_file)) {
  ...
  ggplot(cont_df, aes(x = contraction, y = zscore)) +
    ...
    geom_hline(yintercept = c(-2, 2), color = "#E69F00",
               linetype = "dotted", linewidth = 0.5) +
    facet_wrap(~parameter, nrow = 1) +
    labs(
      title    = "NPE Posterior Z-score vs Contraction (Model A, static scenario)",
      subtitle = "Well-calibrated: points spread horizontally, z-scores within ±2 dashed lines.",
      x = "Posterior contraction  (1 − post. SD / prior SD)",
      y = "Posterior z-score  (post. mean − truth) / post. SD"
    ) +
    theme_bw()
} else {
  cat("> `contraction_data.csv` not found. Re-run the updated `amortized_particle_filter.py`.\n")
}
```
...
if (file.exists(rec_file)) {
  ...
  ggplot(rec_df, aes(x = true_val, y = post_mean)) +
    geom_pointrange(aes(ymin = post_q05, ymax = post_q95),
                    alpha = 0.35, color = "#0072B2", linewidth = 0.4) +
    geom_abline(slope = 1, intercept = 0,
                color = "red", linetype = "dashed", linewidth = 0.7) +
    facet_wrap(~parameter, scales = "free", nrow = 1) +
    labs(
      title    = "NPE Parameter Recovery (Model A, static scenario, 300 test subjects)",
      subtitle = "Posterior mean ± 90% CI vs prior-drawn truth. Points on diagonal = unbiased recovery.",
      x = "True parameter value (drawn from prior)",
      y = "Posterior mean"
    ) +
    theme_bw()
} else {
  cat("> `recovery_data.csv` not found. Re-run the updated `amortized_particle_filter.py`.\n")
}
```

**Reading contraction and recovery.** $\varepsilon$ should show noticeable contraction (> 0.25 on a Kruschke-length sequence) and the recovery scatter should cluster around the diagonal. $N_{\text{particles}}$ contracts less — a single short sequence is only loosely informative about working-memory capacity. $\mu$ contracts very little on static data by design: the particle filter trajectory on a static task provides no signal about mutation rate, so the posterior correctly stays near the prior. The compression toward the prior centre in $\mu$'s recovery scatter is the right answer, not a failure.

### The Main Empirical Result: NPE vs Stan Posteriors

Here is the comparison the chapter has been building toward. To compare like-for-like, both models must be fitted to the *same* observed sequence — the demo subject's response vector that the BayesFlow script also wrote out, and the demo subject's true $\varepsilon$ that we use as the reference line. Without that shared dataset the gap conflates two effects (structural bias and different observed data); the v2 script therefore emits `demo_responses.csv` (the response sequence + per-trial features and feedback) and `demo_truth.csv` (the true $\varepsilon$, $N_{\text{particles}}$, $\mu$ for that subject) alongside `posterior_eps.csv`. The chunk below reads both, refits Model B (via HMC) on the demo subject's responses, and overlays its posterior on the NPE posterior. **The gap between the two distributions is the structural bias you accept when you replace the cognitive theory with a tractable proxy.**

```{r ch15_npe_vs_stan, eval=npe_artifacts_available}
demo_resp_path  <- file.path(npe_artifacts_dir, "demo_responses.csv")
demo_truth_path <- file.path(npe_artifacts_dir, "demo_truth.csv")

have_resp  <- file.exists(demo_resp_path)
have_truth <- file.exists(demo_truth_path)

if (have_resp && exists("npe_eps") && exists("mod_rule_stan") && !is.null(mod_rule_stan)) {
  demo_resp_df <- readr::read_csv(demo_resp_path, show_col_types = FALSE)
  # Expected columns: y, height, position, feedback.
  stan_dat_demo <- list(
    ntrials                = nrow(demo_resp_df),
    nfeatures              = 2L,
    y                      = as.integer(demo_resp_df$y),
    obs                    = as.matrix(demo_resp_df[, c("height", "position")]),
    prior_logit_error_mean = LOGIT_ERROR_PRIOR_MEAN,
    prior_logit_error_sd   = LOGIT_ERROR_PRIOR_SD
  )
  fit_demo <- mod_rule_stan$sample(
    data = stan_dat_demo, seed = 2027,
    chains = 2, parallel_chains = 2,
    iter_warmup = 800, iter_sampling = 1000,
    refresh = 0, adapt_delta = 0.9
  )
  stan_eps_demo <- as_draws_df(fit_demo$draws("error_prob"))$error_prob

  truth_eps <- if (have_truth)
    readr::read_csv(demo_truth_path, show_col_types = FALSE)$eps[1] else NA_real_

  comparison_df <- bind_rows(
    tibble(eps = npe_eps,       method = "Model A (NPE, particle filter)"),
    tibble(eps = stan_eps_demo, method = "Model B (HMC, marginalised mixture)")
  )

  p_cmp <- ggplot(comparison_df, aes(x = eps, fill = method, color = method)) +
    geom_density(alpha = 0.4, linewidth = 0.6) +
    ...
    labs(
      title    = "NPE vs HMC: posterior on ε for the demo subject",
      subtitle = if (!is.na(truth_eps))
                   sprintf("Red dashed: demo subject's true ε = %.3f. Gap between distributions = structural bias of the proxy.", truth_eps)
                 else "Demo truth not available; gap = structural bias of the proxy.",
      x = "ε", y = "Density"
    )
  if (!is.na(truth_eps))
    p_cmp <- p_cmp + geom_vline(xintercept = truth_eps, color = "red", linetype = "dashed")
  print(p_cmp)

  cat("\n--- Posterior summaries (demo subject) ---\n")
  cat("Model A (NPE) posterior:    mean =", round(mean(npe_eps), 4),
      "  95% CI = [", round(quantile(npe_eps, 0.025), 4), ",",
                       round(quantile(npe_eps, 0.975), 4), "]\n")
  cat("Model B (HMC) posterior:    mean =", round(mean(stan_eps_demo), 4),
      "  95% CI = [", round(quantile(stan_eps_demo, 0.025), 4), ",",
                       round(quantile(stan_eps_demo, 0.975), 4), "]\n")
  if (!is.na(truth_eps))
    cat(sprintf("Demo subject's true ε:      %.3f\n", truth_eps))
} else {
  cat("> NPE-vs-HMC overlay needs `demo_responses.csv` and `demo_truth.csv`",
      "next to the BayesFlow artifacts. Re-run `amortized_particle_filter.py`",
      "after the v2 patch that emits these files.\n")
}
```

### Demonstrating That the Gap Tracks Rule-Set Mismatch

The headline claim — "the gap is structural bias, not inference noise" — requires an experiment. The standalone script can simulate datasets at multiple schedule seeds, fit each via NPE (a single forward pass per dataset) and via HMC (a Stan refit per dataset), and write a `gap_results.csv` with columns `seed, true_eps, npe_mean_eps, stan_mean_eps, gap`. The chunk below loads and plots that file when present.

```{r ch15_gap_demonstration}
gap_csv <- file.path(npe_artifacts_dir, "gap_results.csv")
if (npe_artifacts_available && file.exists(gap_csv)) {
  gap_results <- readr::read_csv(gap_csv, show_col_types = FALSE)
  print(gap_results)

  ggplot(gap_results, aes(x = factor(seed), y = gap)) +
    geom_col(fill = "#E69F00", alpha = 0.7) +
    labs(title    = "NPE-vs-HMC gap across different schedule seeds",
         subtitle = "Large gap = converged rule NOT in Model B's set. Small gap = converged rule IS in set.",
         x = "Schedule seed", y = "|NPE mean ε − HMC mean ε|")
} else {
  cat("> `gap_results.csv` not found. To populate this section, extend",
      "`amortized_particle_filter.py` to (i) loop over schedule seeds,",
      "(ii) call `workflow.sample()` per seed, (iii) refit Model B per seed",
      "from R or Stan, and (iv) write `gap_results.csv` to the static/ directory.\n")
}
```
...
```{r ch15_amortization_demo, eval=FALSE}
...
In v2, a *separate* network is trained per scenario by `amortized_particle_filter.py --scenario {static,contingent_shift,drift}`, and each writes its outputs (including a `posterior_demo.csv` for that scenario's demo subject and an optional `demo_truth.csv`) into its own subdirectory under `simdata/ch15_v2_bayesflow/`. The chunk below loads the three `posterior_demo.csv` files and overlays the resulting $\mu$ posteriors. (The single-network-across-scenarios story is also workable — the embedding is conditioned on the per-trial feedback sequence — but training one network per scenario is cleaner and is what the v2 script does.)

```{r ch15_npe_param_recovery_nonstationary}
all_have_demo <- all(vapply(npe_scenarios,
  function(s) file.exists(file.path(npe_artifacts_root, s, "posterior_demo.csv")),
  logical(1)))

if (all_have_demo) {
  scen_post <- map_dfr(npe_scenarios, function(s) {
    df <- readr::read_csv(file.path(npe_artifacts_root, s, "posterior_demo.csv"),
                          show_col_types = FALSE)
    df$scenario <- s
    df
  })

  truth_per_scen <- map_dfr(npe_scenarios, function(s) {
    p <- file.path(npe_artifacts_root, s, "demo_truth.csv")
    if (file.exists(p)) {
  ...
  p_mu_scen <- ggplot(scen_post, aes(x = mu, fill = scenario, color = scenario)) +
    ...
    labs(title    = "NPE μ posterior across scenarios (one demo subject per scenario)",
         subtitle = "Grey dashed = prior. Per-scenario demo truth (if available) overlaid as vertical lines.",
         x = "μ", y = "Density")

  if (nrow(truth_per_scen) > 0) {
    p_mu_scen <- p_mu_scen +
      geom_vline(data = truth_per_scen, aes(xintercept = mu, color = scenario),
  ...
  scen_post |>
    ...
    knitr::kable(caption = "Demo-subject NPE posterior on μ per scenario.")
} else {
  cat("> Per-scenario `posterior_demo.csv` files not all present under",
      "`simdata/ch15_v2_bayesflow/`. Run the standalone NPE script for each",
      "of the three scenarios (`static`, `contingent_shift`, `drift`) and",
      "copy outputs into their respective subdirectories.\n")
}
```

**Reading the demonstration.** Each scenario's demo subject is drawn from the prior, so the per-scenario truth (vertical dashed lines, when available from `demo_truth.csv`) is *not* a fixed value across scenarios. The substantive prediction is about the *width* of the $\mu$ posterior relative to the grey prior: on `static` it should sit close to the prior (no information about mutation), and on `contingent_shift` / `drift` it should be visibly narrower. This is the parameter-recovery counterpart of the calibration block above: `metrics.csv` tells us the network is calibrated; this plot tells us *when* the data are informative enough for that calibration to translate into useful contraction.
...
if (all_have_demo) {
  ...
  truth_per_scen <- map_dfr(npe_scenarios, function(s) {
    p <- file.path(npe_artifacts_root, s, "demo_truth.csv")
    if (file.exists(p)) {
      df <- readr::read_csv(p, show_col_types = FALSE); df$scenario <- s; df
    } else tibble()
  })
  ...
  scenario_labels <- c(static = "Static Kruschke",
                       contingent_shift = "Contingent shift",
                       drift = "Smooth drift")

  p_eps_scen <- ggplot(scen_post, aes(x = eps, fill = scenario, color = scenario)) +
    geom_density(alpha = 0.35, linewidth = 0.7) +
    geom_density(data = prior_df, aes(x = eps),
                 inherit.aes = FALSE, linetype = "dashed",
                 color = "grey30", linewidth = 0.5) +
    scale_fill_viridis_d(option = "plasma", labels = scenario_labels, name = "Scenario") +
    scale_color_viridis_d(option = "plasma", labels = scenario_labels, name = "Scenario") +
    labs(title    = "NPE posterior for ε across scenarios (one demo subject per scenario)",
         subtitle = "Grey dashed = prior. Per-scenario demo truth (if available) shown as vertical lines.",
         x = "ε", y = "Density") +
    theme(legend.position = "bottom")
  ...
  p_mu_scen <- ggplot(scen_post, aes(x = mu, fill = scenario, color = scenario)) +
    ...
    labs(title    = "NPE posterior for μ across scenarios",
         subtitle = "μ contracts on non-stationary data; remains prior-like on static",
         x = "μ", y = "Density") +
    theme(legend.position = "bottom")

  if (nrow(truth_per_scen) > 0) {
    p_eps_scen <- p_eps_scen +
      geom_vline(data = truth_per_scen,
                 aes(xintercept = eps, color = scenario),
  ...
  cat("> Per-scenario `posterior_demo.csv` files missing under `simdata/ch15_v2_bayesflow/`.\n")
}
```
...
```{r ch15_npe_prior_post_scenarios}
all_have_demo <- all(vapply(npe_scenarios,
  function(s) file.exists(file.path(npe_artifacts_root, s, "posterior_demo.csv")),
  logical(1)))
...
if (all_have_demo) {
  ...
  n_draws_prior <- 3000L
  prior_eps_draws <- plogis(rnorm(n_draws_prior, LOGIT_ERROR_PRIOR_MEAN, LOGIT_ERROR_PRIOR_SD))
  prior_mu_draws  <- plogis(rnorm(n_draws_prior, LOGIT_MU_PRIOR_MEAN,  LOGIT_MU_PRIOR_SD))
  ...
  truth_for <- function(key, col) {
    if (nrow(truth_per_scen) == 0) return(NA_real_)
    v <- truth_per_scen[[col]][truth_per_scen$scenario == key]
    if (length(v) == 0) NA_real_ else v[1]
  }

  make_panel <- function(prior_vec, posterior_vec, scenario_label, true_val, xlab) {
    df_all <- bind_rows(
      tibble(value = posterior_vec, type = "Posterior"),
      tibble(value = prior_vec,     type = "Prior")
    )
    p <- ggplot(df_all, aes(x = value, fill = type, color = type)) +
      ...
      labs(title = scenario_label, x = xlab, y = "Density") +
      theme(legend.position = "none", plot.title = element_text(size = 10))
    if (!is.na(true_val))
      p <- p + geom_vline(xintercept = true_val, color = "red", linetype = "dashed")
    p
  }

  eps_panels <- lapply(sc_list, function(sc) {
    post_sc <- scen_post |> dplyr::filter(scenario == sc$key)
    make_panel(prior_eps_draws, post_sc$eps, sc$label, truth_for(sc$key, "eps"), "ε")
  })
  mu_panels <- lapply(sc_list, function(sc) {
    post_sc <- scen_post |> dplyr::filter(scenario == sc$key)
    make_panel(prior_mu_draws, post_sc$mu, sc$label, truth_for(sc$key, "mu"), "μ")
  })
  ...
  print(
    top_row / bottom_row +
      plot_annotation(
        ...
        subtitle = "Row 1: ε  |  Row 2: μ  |  blue = posterior, grey = prior, red dashed = per-scenario demo truth",
        theme    = theme(plot.title = element_text(face = "bold"))
      )
  )
} else {
  cat("> Per-scenario `posterior_demo.csv` files missing.\n")
}
```
...
The recovery scatter shows posterior mean ± 90% CI against the prior-drawn truth across 300 held-out test subjects, separately for each scenario. It is a denser and more honest summary than a hand-picked parameter sweep: the truths span the full prior range, so deviations from the diagonal reveal exactly where in parameter space the network struggles.

```{r ch15_npe_recovery_sweep_artifact}
all_have_recov <- all(vapply(npe_scenarios,
  function(s) file.exists(file.path(npe_artifacts_root, s, "recovery_data.csv")),
  logical(1)))

if (all_have_recov) {
  rec_all <- map_dfr(npe_scenarios, function(s) {
    read_csv(file.path(npe_artifacts_root, s, "recovery_data.csv"),
             show_col_types = FALSE) |>
      mutate(scenario = s)
  }) |>
    mutate(
  ...
  ggplot(rec_all, aes(x = true_val, y = post_mean)) +
    geom_pointrange(aes(ymin = post_q05, ymax = post_q95),
                    alpha = 0.25, color = "#0072B2", linewidth = 0.35) +
    geom_abline(slope = 1, intercept = 0,
                color = "red", linetype = "dashed", linewidth = 0.7) +
    facet_grid(scenario ~ parameter, scales = "free") +
    labs(
      title    = "NPE Parameter Recovery Across Scenarios (Model A, 300 test subjects per scenario)",
      subtitle = "Posterior mean ± 90% CI vs prior-drawn truth. Red diagonal = perfect recovery.",
      x = "True parameter value (drawn from prior)",
      y = "Posterior mean"
    ) +
    theme_bw() +
  ...
  metrics_all <- map_dfr(npe_scenarios, function(s) {
    p <- file.path(npe_artifacts_root, s, "metrics.csv")
    if (file.exists(p)) {
      readr::read_csv(p, show_col_types = FALSE) |> mutate(scenario = s)
    } else tibble()
  })
  if (nrow(metrics_all) > 0) {
    metrics_all |>
      dplyr::rename(metric = 1) |>
      dplyr::select(scenario, metric, dplyr::everything()) |>
      knitr::kable(
        caption = "NPE per-scenario diagnostic metrics (NRMSE, log-gamma, calibration error, posterior contraction).",
        digits = 3)
  }
} else {
  cat("> Per-scenario `recovery_data.csv` not all present.",
      "Re-run the updated `amortized_particle_filter.py` for each scenario.\n")
}
```

```{r ch15_npe_recovery_sweep_legacy, eval=FALSE, include=FALSE}
...
The one-shot recovery scatter tests accuracy. SBC is the formal calibration test: under data drawn from the prior predictive, the posterior ranks of the true parameters should be uniform. Running SBC separately for each scenario isolates *which* environment reveals miscalibration. The `sbc_ranks.csv` exported by the updated script carries integer ranks 0–99 (from 99 posterior draws) for each of 300 test subjects.

```{r ch15_npe_sbc_scenarios_artifact}
all_have_sbc <- all(vapply(npe_scenarios,
  function(s) file.exists(file.path(npe_artifacts_root, s, "sbc_ranks.csv")),
  logical(1)))

if (all_have_sbc) {
  sbc_all <- map_dfr(npe_scenarios, function(s) {
    read_csv(file.path(npe_artifacts_root, s, "sbc_ranks.csv"),
             show_col_types = FALSE) |>
      mutate(scenario = s)
  }) |>
    mutate(
  ...
  n_ranks <- nrow(sbc_all) / n_distinct(sbc_all$parameter) / n_distinct(sbc_all$scenario)
  n_bins  <- 10L

  ggplot(sbc_all, aes(x = rank)) +
    geom_histogram(bins = n_bins, fill = "#0072B2", color = "white", alpha = 0.8) +
    geom_hline(yintercept = n_ranks / n_bins, color = "red",
               linetype = "dashed", linewidth = 0.7) +
    facet_grid(scenario ~ parameter) +
    labs(
      title    = "NPE SBC Rank Histograms Across Scenarios (Model A, 300 test subjects)",
      subtitle = "Red dashed = uniform expectation. Flat = calibrated. μ static column expected ≈ flat for correct reasons.",
      x = "Rank of true value in 99 posterior draws", y = "Count"
    ) +
    theme_bw() +
    theme(strip.text = element_text(size = 9))
} else {
  cat("> Per-scenario `sbc_ranks.csv` not all present.",
      "Re-run the updated `amortized_particle_filter.py` for each scenario.\n")
}
```

```{r ch15_npe_sbc_scenarios_legacy, eval=FALSE, include=FALSE}
...
Rank histograms test calibration (correct uncertainty). The z-score versus contraction scatter tests both simultaneously: well-calibrated posteriors produce z-scores that are approximately standard normal (contained within ±2 dashed lines) and a spread of contraction values reflecting genuine information gain. The `contraction_data.csv` exported by the updated script carries these quantities across 300 test subjects per scenario.

```{r ch15_npe_sbc_recovery_plots_artifact}
all_have_cont <- all(vapply(npe_scenarios,
  function(s) file.exists(file.path(npe_artifacts_root, s, "contraction_data.csv")),
  logical(1)))

if (all_have_cont) {
  cont_all <- map_dfr(npe_scenarios, function(s) {
    read_csv(file.path(npe_artifacts_root, s, "contraction_data.csv"),
             show_col_types = FALSE) |>
      mutate(scenario = s)
  }) |>
    mutate(
  ...
  ggplot(cont_all, aes(x = contraction, y = zscore)) +
    ...
    labs(
      ...
      subtitle = "Well-calibrated posteriors: z-scores within ±2 (orange), contraction spread reflects information gain.",
      x = "Posterior contraction  (1 − post. SD / prior SD)",
      y = "Posterior z-score  (post. mean − truth) / post. SD"
    ) +
    theme_bw() +
  ...
  cat("> Per-scenario `contraction_data.csv` not all present.",
      "Re-run the updated `amortized_particle_filter.py` for each scenario.\n")
}
```

```{r ch15_npe_sbc_recovery_plots_legacy, eval=FALSE, include=FALSE}
if (!is.null(sbc_scenario_results)) {
