...
# Label only rules that persist ≥ 2 trials; place label at each block's peak weight
rule_labels <- rule_tracking_plot_data |>
  group_by(rule_block, rule_text) |>
  filter(n() >= 2) |>
  slice_max(weight, n = 1, with_ties = FALSE) |>
  ungroup() |>
  mutate(rule_text = str_wrap(rule_text, width = 35))
...
ggplot(rule_tracking_plot_data, aes(x = trial, y = weight)) +
  ...
  geom_text(
    data = rule_labels,
    aes(label = rule_text),
    hjust = 0, vjust = -0.3, size = 2.8, lineheight = 0.9
  ) +
  scale_y_continuous(limits = c(0, NA), name = "Weight (Probability) of Top Rule") +
...
if (regenerate_simulations || !file.exists(npe_model_file)) {
  if (run_intensive_checks) {
    # Convert to PyTorch tensors and move to the selected device (MPS or CPU).
    # On M1 with MPS this is the main speedup: training runs on the GPU.
    theta_tensor <- torch$tensor(train_thetas, dtype = torch$float32)$to(npe_device)
    ...
    x_tensor <- torch$tensor(x_mat, dtype = torch$float32)$to(npe_device)
    ...
    npe_posterior <- inference_obj$build_posterior(
      ...
      mcmc_parameters = list(warmup_steps = 100L)
    )
