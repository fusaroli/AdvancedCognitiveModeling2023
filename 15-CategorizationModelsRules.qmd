...
mod_rule_stan <- cmdstan_model(stan_file_rule, force_recompile = TRUE)
...
if (run_intensive_checks) {
  ...
  cat("Skipping NPE setup (run_intensive_checks is FALSE).\n")
}
```
...
We infer three parameters jointly:
...
* $\mu$ — mutation rate, on the logit scale for unconstrained inference

Adding $\mu$ to the NPE prior is the natural completion of the model. $\varepsilon$ governs how noisily the agent applies its current rule; $\mu$ governs how readily it abandons that rule and draws a fresh hypothesis. They play orthogonal roles: a high-$\varepsilon$ agent responds noisily even when its rule is good; a high-$\mu$ agent responds accurately trial-by-trial but replaces hypotheses frequently. On static data we expect $\mu$ to be poorly identified — matching $q$ in Chapter 13 and $\lambda$ in Chapter 12 on their own static scenarios. On non-stationary data the learning-curve signature (sawtooth depth, recovery speed) depends specifically on $\mu$ in a way that $\varepsilon$ cannot mimic.

Note that $\mu$ has **no counterpart in Model B**. Model B is a static mixture with no dynamics; the concept of mutation rate is structurally absent. The NPE-vs-Stan posterior comparison therefore involves $\varepsilon$ only — Model B simply has nothing to say about $\mu$.
...
```{r ch14_npe_simulator, eval=run_intensive_checks}
...
simulate_model_a <- function(theta_vec) {
  ...
  # Use the scenario-aware filter so mutation_prob is respected.
  # We run the static scenario here (schedule is fixed); for non-stationary
  # validation swap to "contingent_shift" or "drift" as needed.
  sim <- rule_particle_filter_scenario(
...
```{r ch14_npe_training_data, eval=run_intensive_checks}
...
if (regenerate_simulations || !file.exists(npe_cache_file)) {
  ...
  # Draw three parameters from their priors.
  # logit(eps) ~ Normal(LOGIT_ERROR_PRIOR_MEAN, LOGIT_ERROR_PRIOR_SD)
  # log(N)     ~ Uniform(log(20), log(300))     — log-uniform over [20, 300]
  # logit(mu)  ~ Normal(LOGIT_MU_PRIOR_MEAN, LOGIT_MU_PRIOR_SD)
...
```{r ch14_npe_train, eval=run_intensive_checks}
if (run_intensive_checks) {
  ...
  if (regenerate_simulations || !file.exists(npe_model_file)) {
    ...
    # We use a BoxUniform over the joint support of all three parameters.
    # The bounds are set at ±3 SD for the normal priors and the exact
    # range for the log-uniform N_particles prior.
    # theta = [logit(eps), log(N_particles), logit(mu)]
    prior_low  <- torch$tensor(
      c(LOGIT_ERROR_PRIOR_MEAN - 3 * LOGIT_ERROR_PRIOR_SD,
        log(20),
        LOGIT_MU_PRIOR_MEAN    - 3 * LOGIT_MU_PRIOR_SD),
      dtype = torch$float32
    )
...
```{r ch14_npe_inference, eval=run_intensive_checks}
...
# ── Marginal SBC ──────────────────────────────────────────────────────────────
# Three parameters: logit(eps), log(N_particles), logit(mu).
# We expect logit(eps) and log(N_particles) to show uniform rank histograms
# on static data. logit(mu) is expected to be poorly identified on static
# data — its rank histogram may be flat (uniformly uncertain) or slightly
# humped (posterior matches prior shape). This is the correct behaviour and
# does not indicate miscalibration; it indicates non-identifiability.
...
```{r ch14_gap_demonstration, eval=run_intensive_checks}
...
The three levels represent different cognitive theories:
...
Each level adds a structural commitment. Levels 1 and 3 introduce new parameters or data-dependent quantities that themselves need prior justification. Whether targeted mutation fits human data better than prior-sampling mutation is an open empirical question, but implementing all variants allows forward simulation of qualitatively different learner types.
...
# per trial of being mutated at all. The three targeted types differ only in
...
mutate_particles_targeted <- function(particles, weights,
  ...
  # For feature-weighted, compute the feature distribution once per call
  # from the current high-weight particles — not from the particles being
  # replaced (which may be low-weight ones).
...
* Level 3 (`feature_weighted`) is the most cognitively interesting but also the most dependent on $N_\text{particles}$: with very few particles the feature counts are noisy and the bias is unreliable. A floor of 0.5 per feature prevents any dimension from being completely excluded from the mutation distribution.
* All three targeted variants are more conservative than prior-sampling: they keep more of the current rule intact, which means they explore less of the rule space per unit time. On a task where the true rule is very different from anything currently in the particle set, targeted mutation may be slower to recover than prior-sampling.
...
rule_particle_filter_scenario <- function(schedule,
  ...
  for (i in seq_len(n_trials)) {
    ...
    # strategies ("threshold", "component", "feature_weighted").
