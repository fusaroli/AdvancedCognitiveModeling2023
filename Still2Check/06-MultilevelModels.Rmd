...
pacman::p_load(
  ...
  pathfinder       # Multi-path variational inference for NUTS initialization
)
...
Traditional statistical approaches to handling data from multiple individuals often force a false dichotomy between two extremes, both of which impose rigid, unjustified structural assumptions on the data:

1. **Complete Pooling (Underfitting)**: Treats all participants as identical by averaging or combining their data. This assumes the population variance (the average error in predicting an individual from the population) is strictly zero ($\sigma \to 0$). Complete pooling estimates a single set of parameters for the entire group, completely ignoring potentially meaningful individual variation. Advantage: Highly stable estimates due to maximum data density. Disadvantage: Systematically fails to capture actual cognitive heterogeneity, often describing an "average" participant that does not exist. Example: Full pooling corresponds to fitting one generative model to a concatenated vector of all participants' choices.

2. **No Pooling (Overfitting)**: Analyzes each participant in complete isolation from the others, estimating independent parameters ($\theta_j$) for each individual $j$. This de fact assumes the population variance is infinite ($\sigma \to \infty$), failing to leverage any shared structural information across the cohort and can lead to noisy or unstable estimates, especially for individuals with limited data. Advantage: Unconstrained capacity to capture individual variation (which can be a problem given the noisy nature of the data). Disadvantage: Severely inefficient and highly vulnerable to noise. For individuals with sparse or noisy trial data, the posterior estimates will be excessively wide or completely dominated by the likelihood's random fluctuations. 
No pooling corresponds to fitting $J$ separate models to $J$ individual participants.

### The Solution: Partial Pooling (Multilevel Modeling)

Multilevel modeling offers a principled, data-driven middle ground through partial pooling. Instead of forcing $\sigma$ to be $0$ or $\infty$, we treat the population variance as a free parameter to be estimated from the data. We structure the model hierarchically:
...
This hierarchical structure permits a flow of information between levels:

* The estimation of an individual's parameter ($\theta_j$) is simultaneously constrained by their own empirical data (the likelihood) and the population distribution (the hierarchical prior).

* *Shrinkage*: Estimates for individuals with sparse or noisy data are precision-weighted; they are "shrunk" towards the population mean $\mu$. This is not an artifact; it is a mathematical guarantee that regularizes inference, preventing overfitting and yielding more robust, out-of-sample optimal estimates.

* The model simultaneously estimates (jointly marginalizes) individual differences and overall population tendencies. 

We must always ask deep structural questions here: Are we expecting heavy tails in our population? If some agents employ fundamentally different heuristic strategies, a Normal population prior might over-shrink extreme but genuine parameter values.Consider our Matching Pennies cognitive task: players will invariably differ in their baseline bias ($\theta$), memory sensitivity ($\beta$), or strategic sophistication. Multilevel modeling quantifies this population variance ($\sigma$) while anchoring the cohort's central tendency ($\mu$).
...
\begin{tikzpicture}[
    ...
    pics/bern_dist/.style={
        code={
            ...
            \draw[thick, blue, ycomb, line width=4pt] plot coordinates { (-0.4, 0.3) (0.4, 0.8) };
    ...
    % 1. Population Level
    \node[continuous, latent] (mu) at (-1.5, 0) {$\mu_\theta$};
    \node[continuous, latent] (sigma) at (1.5, 0) {$\sigma_\theta$};
    
    % 2. Individual Level
    \node[continuous, latent] (theta) at (0, -3) {$\theta_{\text{logit}, j}$};
    
    % 3. Data Level
    \node[discrete, observed] (h) at (0, -6) {$h_{ij}$};

    % Plates
    \node[plate, fit=(h), label={[anchor=south east, font=\small\bfseries]below right:$i=1 \dots N$}] (plate_N) {};
    \node[plate, inner sep=15pt, fit=(theta) (plate_N), label={[anchor=south east, font=\small\bfseries]below right:$j=1 \dots J$}] (plate_J) {};

    % Edges
    \draw[stochastic] (mu) -- (theta);
    \draw[stochastic] (sigma) -- (theta);
    \draw[stochastic] (theta) -- (h);


    % =========================================================
    % RIGHT SIDE: KRUSCHKE-STYLE DISTRIBUTIONAL NOTATION
    % =========================================================
    \node[font=\large\bfseries] at (8, 2) {Kruschke-Style Notation};

    % 1. Population Level (Hyperparameters)
    \coordinate (k_mu) at (6, 0);
    \draw (k_mu) pic {norm_dist};
    \node[above=0.8cm of k_mu, font=\bfseries] {Pop. Mean ($\mu_\theta$)};

    \coordinate (k_sigma) at (10, 0);
    \draw (k_sigma) pic {exp_dist};
    \node[above=0.8cm of k_sigma, font=\bfseries] {Pop. SD ($\sigma_\theta$)};

    % 2. Individual Level (Latent Parameters)
    \coordinate (k_theta) at (8, -3);
    \draw (k_theta) pic {norm_dist};
    \node[above=0.8cm of k_theta, font=\bfseries] {Agent Bias ($\theta_{\text{logit}, j}$)};

    % 3. Data Level (Observed)
    \coordinate (k_h) at (8, -6);
    \draw (k_h) pic {bern_dist};
    \node[above=0.8cm of k_h, font=\bfseries] {Choice ($h_{ij}$)};

    % Kruschke Plates (Rounded rectangles drawn explicitly around the icons)
    \draw[plate, dashed] (6.5, -4.5) rectangle (9.5, -2) node[below left, font=\small\bfseries] {$j=1 \dots J$};
    \draw[plate, dotted] (6.5, -7.5) rectangle (9.5, -5) node[below left, font=\small\bfseries] {$i=1 \dots N$};

    % Edges with tildes
    \draw[stochastic] (k_mu) -- (k_theta) node[midway, left, font=\large] {$\sim$};
    \draw[stochastic] (k_sigma) -- (k_theta) node[midway, right, font=\large] {$\sim$};
    \draw[stochastic] (k_theta) -- (k_h) node[midway, right, font=\large] {$\sim$};
