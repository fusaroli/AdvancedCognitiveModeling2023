...
>
> In our model, both prototypes are initialised at $\vec{\mu}_0 = \vec{\mu}_1 = (2.5, 2.5)$ with isotropic covariance $\Sigma_0 = \Sigma_1 = 10 \cdot I$. The prediction step on trial 1 adds $Q = q \cdot I$, yielding $\Sigma^- = (10 + q) \cdot I$ for both categories. The decision rule then compares two **identical** multivariate normal densities at the observed stimulus. Their log-densities are equal, the softmax reduces to $0.5$, and the agent chooses Category 1 with probability exactly $0.5$ regardless of which stimulus is presented.
>
> **This is a cognitive commitment, not a technical default.** The model is claiming that humans, encountering the very first stimulus of a categorisation task with no prior information, are perfectly indifferent. If you wanted to relax it, the cleanest extension is a separate `bias` parameter on the logit scale.
>
> **What we commit to here:** No category bias. Trial 1 is by construction $P(\text{cat 1}) = 0.5$.

> **Q2 — The Persistence Question.** *Once the prototypes are well-estimated, does the agent forget? Does the model ever reset?*
>
> We implement the Kalman filter with a **prediction step** at each trial that adds a process-noise covariance $Q = q \cdot I$ to both category covariances before the decision and update:
>
> $$\Sigma^-_{t,C} = \Sigma_{t-1,C} + Q \quad \text{(prediction step, applied to both categories)}$$
>
> This means uncertainty no longer monotonically decreases. At each trial, the prediction step increases covariances by $q$ (prototype may have drifted), and the measurement update shrinks them via the Kalman gain. At steady state, both forces balance and uncertainty converges to a fixed value determined by $q$ and $r$ jointly.
>
> Cognitively, $q$ quantifies **prototype instability**: small $q$ means the category's central tendency is approximately fixed; large $q$ means it can shift substantially between observations and the model must continuously relearn. This makes the model appropriate for category-drift studies, concept revision tasks, and any scenario where the ideal category boundary changes over time.
>
> **Identifiability warning.** With the static Kruschke (1993) paradigm (same 8 stimuli cycling through 8 blocks), `log_q` and `log_r` are partially confounded: both affect the effective likelihood width at decision time and the steady-state accuracy plateau. `log_q` is best identified from data where performance *oscillates* or *recovers* following a structural change in the environment. The SBC (§"Simulation-Based Calibration") reveals the extent of this confound under the static paradigm we use here.
>
> **What we commit to here:** Isotropic process noise $Q = q \cdot I$, constant across trials and identical for both categories. The drift rate $q$ is a free parameter estimated from data.

> **Q3 — The Observation-vs-Decision Noise Question.** *Where does response stochasticity come from?*
>
> Our model has two noise parameters, but $r$ is still doing double duty. It controls:
>
...
>
> The new parameter $q$ adds a third role: **how much prototype uncertainty grows back** between trials. This partially separates steady-state uncertainty (set by the $q$/$r$ balance) from the sharpness of learning transitions (set mainly by $r$ at early trials). A more flexible model would add a separate softmax temperature $\tau$; we omit it here for parsimony, but the Phase 0 commitment should be named.
>
> **What we commit to here:** $r$ conflates observational and decisional noise; $q$ governs drift. The pair $(r, q)$ is better identified from learning *trajectories* with category drift than from static asymptotic performance alone.

> **Q4 — The Identifiability Question.** *Are there other parameters we are silently fixing?*
>
> Three structural choices ride along with this model:
>
...
> 2. **Initial covariance** $\Sigma_0 = \Sigma_1 = 10 \cdot I$. Large enough that the first observation dominates the first update. This makes the model "open-minded" at trial 1, but the specific value of 10 is a free choice that we do not infer.
> 3. **Diagonal $R$ and $Q$**. We assume measurement noise and process noise are both independent across features. A real perceptual system might have correlated noise across feature dimensions.
>
> Each of these choices is a structural prior. None of them is inferred from data. The reader should know they are choices.

**Summary of Phase 0 commitments.** We are fitting a *two-parameter* model: $r$ controls both Kalman gain and decisional precision; $q$ controls prototype drift speed and steady-state uncertainty. Both are inferred from data on the log scale. There is no category bias, fixed initial means, fixed initial covariances, and diagonal noise structures for both $R$ and $Q$. The Stan implementation in §"The Prototype Model in Stan" implements exactly this.
...
ggplot(prototype_responses,
  ...
  facet_wrap(~paste0("q = ", q_value), ncol = 3) +
  labs(
...
* **Phase 0 commitments are implemented in code**, not buried: the trial-1 coin flip is structural (identical initial means and covariances), the same `r_value` is used in both the Kalman update and the decision step, and `q_value` controls prototype drift speed.
...
```{r ch13_lfo_run, eval=run_intensive_checks}
...
```{r ch13_precision_analysis, eval=run_intensive_checks}
...
```{r ch13_sbc_prototype, eval=run_intensive_checks}
...
if (regenerate_simulations || !file.exists(sbc_proto_filepath)) {
  ...
  sbc_results_proto <- compute_SBC(
    proto_sbc_generator,
    proto_sbc_backend,
    cache_path = here("simdata", "ch13_sbc_proto_cache"),
    keep_fits  = FALSE
  )
...
```{r ch13_extensive_recovery_ml, eval=run_intensive_checks}
...
```{r ch13_sbc_multilevel, eval=run_intensive_checks}
...
* **Process noise identifiability (Phase 0 Q2)**: The model includes process noise $Q = q \cdot I$, but $q$ and $r$ are partially confounded under the static Kruschke (1993) paradigm — both affect the effective likelihood width at decision time. Separate identifiability requires non-stationary data where prototypes demonstrably shift. The SBC analysis shows the extent of this confound, and the precision analysis quantifies how many trials of category drift are needed to pin down $q$.
