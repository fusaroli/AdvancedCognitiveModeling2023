...
We extend `rule_particle_filter` to take a `mutation_prob` argument and a `scenario` switch that controls how the true feedback sequence is produced. The scenarios mirror Chapters 11 and 12 exactly: static Kruschke, performance-contingent abrupt shifts, and smooth drift.
...
::: {.callout-note title="Scenario order across the three chapters"}
All three chapters use the same three scenarios (static Kruschke, performance-contingent abrupt shifts, smooth drift), but the order in which they are presented differs because the ordering principle differs for each model:

| Chapter | Ordering principle | Order |
|---|---|---|
| Ch. 11 — Decay GCM | Increasing $\lambda$ preference (higher recency = larger effect of non-stationarity) | Static → Drift → Contingent shift |
| Ch. 12 — Kalman prototype | Increasing assumption match (Gaussian drift is the model's structural home) | Static → Contingent shift → Drift |
| Ch. 13 — Rule particle filter | Increasing structural demand on the mutation mechanism | Static → Contingent shift → Drift |

Chapters 12 and 13 share the same presentation order, but for different reasons: the Kalman prototype is *best matched* to drift data (its process-noise assumption generates exactly Gaussian drift), whereas the particle filter uses mutation as a *general-purpose rejuvenation* that works for both non-stationary scenarios. The scenarios are the same across all three chapters; readers should compare the learning-curve patterns across chapters to see how the structural differences between models show up in their sensitivity to non-stationarity.
:::
