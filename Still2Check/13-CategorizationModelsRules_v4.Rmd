---
title: "13 — Rule-Based Models: Learning Explicit Criteria (v4 — NPE replaces ABC)"
output:
  html_document:
    mathjax: default
    toc: true
...
```{r, 13 resample threshold sensitivity, eval=run_intensive_checks}
resample_sensitivity_file <- "simdata/W13_resample_threshold_sensitivity.rds"
...
} else if (file.exists(resample_sensitivity_file)) {
...
```{r, 13 psbc model B, eval=run_intensive_checks}
...
if (!is.null(fit_rule_stan)) {
  ...
  psbc_generator <- SBC_generator_function(
    function(i) {
      ...
      rule_idx <- sample.int(5L, 1)
      sched    <- make_subject_schedule(stimulus_info, n_blocks, seed = 65000 + i)
      obs_mat  <- as.matrix(sched[, c("height", "position")])
      n_trials <- nrow(obs_mat)
  ...
  psbc_results <- compute_SBC(
    psbc_generator, psbc_backend,
    cache_path = "simdata/W13_psbc_cache",
    keep_fits = FALSE
  )
...
```{r, 13 npe setup, eval=run_intensive_checks}
# ── Python environment ────────────────────────────────────────────────────────
...
```{r, 13 npe simulator, eval=run_intensive_checks}
...
```{r, 13 npe training data, eval=run_intensive_checks}
n_train_sims   <- 20000   # 20k sims; increase to 50k for production
npe_cache_file <- "simdata/W13_npe_training_data.rds"
...
```{r, 13 npe train, eval=run_intensive_checks}
npe_model_file <- "simdata/W13_npe_trained_posterior.pkl"
...
```{r, 13 npe inference, eval=run_intensive_checks}
...
```{r, 13 npe validation, eval=run_intensive_checks}
n_cal <- 1000L
...
```{r, 13 npe vs stan, eval=run_intensive_checks}
...
```{r, 13 gap demonstration, eval=run_intensive_checks}
...
```{r, 13 amortization demo, eval=FALSE}
