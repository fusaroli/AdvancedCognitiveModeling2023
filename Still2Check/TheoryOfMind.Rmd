...
# Reading Other Minds: Theory-of-Mind Models
...
- **Execute the six-phase validation battery**: prior predictive
  checks, parameter recovery, Simulation-Based Calibration (SBC),
...
```{r prior-pc-tom0, fig.cap="Prior predictive check for 0-ToM. Each panel shows one simulated agent against a 70/30 biased opponent. Priors should produce heterogeneous but non-pathological choice sequences."}
set.seed(2026)
n_sims <- 9
...
"
writeLines(stan_tom0_single, here::here("stan", "ch8_tom0_single.stan"))
mod_tom0 <- cmdstan_model(here::here("stan", "ch8_tom0_single.stan"))
```
...
```{r recovery-tom0, cache=TRUE}
recovery_path <- here::here("simmodels", "ch8_tom0_recovery.rds")
...
if (regenerate_simulations || !file.exists(recovery_path)) {
  ...
  fit_one <- function(d) {
    fit <- mod_tom0$sample(
      data = list(N = nrow(d), choice = d$choice, op_choice = d$op_choice),
  ...
  recovery_summary <- recovery |>
    ...
    select(agent, log_sigma, log_beta, bias, post) |>
    unnest(post)
...
```{r recovery-tom0-plot, fig.cap="0-ToM parameter recovery. Points = posterior means, bars = 90% CIs, dashed line = identity. Recovery is clean for bias and log_beta; log_sigma is noisier with mild shrinkage to the prior at 120 trials — a design finding we will confirm under the more rigorous SBC next."}
...
prior, simulate data from the likelihood, and compute the rank of
...
```{r sbc-tom0, cache=TRUE}
sbc_path <- here::here("simmodels", "ch8_tom0_sbc.rds")
...
```{r sbc-tom0-plot, fig.width=9, fig.height=5, fig.cap="Simulation-Based Calibration for 0-ToM. Top row: rank histograms (flat = calibrated). Bottom row: ECDF difference (blue line inside grey band = calibrated). log_sigma shows the mildest excursion, consistent with the recovery plot: its posterior is only partially data-driven at 120 trials; log_beta and bias are clean."}
...
`ch8_influence_single.stan` is included in the chapter's
...
"
writeLines(stan_tom1_single, here::here("stan", "ch8_tom1_single.stan"))
mod_tom1 <- cmdstan_model(here::here("stan", "ch8_tom1_single.stan"))
```
...
```{r recovery-tom1, cache=TRUE}
recovery_tom1_path <- here::here("simmodels", "ch8_tom1_recovery.rds")
...
if (regenerate_simulations || !file.exists(recovery_tom1_path)) {
  ...
  recovery1 <- truth1 |>
    ...
    mutate(post = list(fit_one_tom1(log_sigma_op, log_beta_op,
                                    log_beta, bias, agent))) |>
    select(agent, log_sigma_op, log_beta_op, log_beta, bias, post) |>
    unnest(post)
...
```{r recovery-tom1-plot, fig.width=9, fig.height=6, fig.cap="1-ToM parameter recovery. The agent's own log_beta and bias are well recovered. The opponent-model parameters log_sigma_op and log_beta_op are noisier — they enter the likelihood only through the recursion, and a 150-trial sequence only weakly constrains them."}
...
```{r sbc-tom1, cache=TRUE}
sbc_tom1_path <- here::here("simmodels", "ch8_tom1_sbc.rds")
...
```{r sbc-tom1-plot, fig.width=9, fig.height=6, fig.cap="SBC for 1-ToM. Top row: rank histograms. Bottom row: ECDF difference (blue inside grey band = calibrated). log_beta and bias stay inside the band. log_sigma_op and log_beta_op show substantial excursions — the expected empirical signature of an underidentified parameter. The 1-ToM likelihood simply does not have enough contrast in matching-pennies data to pin these opponent-model parameters down across the prior."}
...
powerscale_sensitivity(
  fit, variable = c("log_sigma_op","log_beta_op","log_beta","bias")
) |> print()
...
```{r occam, cache=TRUE}
occam_path <- here::here("simmodels", "ch8_occam.rds")
...
if (regenerate_simulations || !file.exists(occam_path)) {
  ...
  occam_res <- occam_grid |>
    ...
    select(scenario, rep, elpd0, elpd1, delta)
...
```{r write-stan-hier-baseline, cache=TRUE}
stan_tom0_hier <- "
data {
  int<lower=1> N;
  int<lower=1> J;
...
"
writeLines(stan_tom0_hier, here::here("stan", "ch8_tom0_hier.stan"))
mod_tom0_hier <- cmdstan_model(here::here("stan", "ch8_tom0_hier.stan"))
```
...
```{r fit-hier-baseline, cache=TRUE}
hier_path <- here::here("simmodels", "ch8_tom0_hier_fit.rds")
...
```{r write-stan-mixture, cache=TRUE}
stan_mixture_reg <- "
data {
  int<lower=1> N;
  int<lower=1> J;
...
generated quantities {
  ...
  // This is the Occam evidence signal: larger => stronger support for 1-ToM.
  vector[J] ll_diff_j;
...
"
writeLines(stan_mixture_reg, here::here("stan", "ch8_mixture_reg.stan"))
mod_mix <- cmdstan_model(here::here("stan", "ch8_mixture_reg.stan"))
```
...
```{r fit-mixture, cache=TRUE}
mix_path <- here::here("simmodels", "ch8_mixture_reg_fit.rds")
...
```{r mixture-coef-plot, fig.width=10, fig.height=4.5, fig.cap="Left: posterior distributions of the two covariate effects on logit(π_s), the species-level probability of being a 1-ToM agent. Positive = more 1-ToM-compatible as the covariate increases. Right: species-level posterior π_s (dot = median, bar = 90% CI)."}
mix_coef <- fit_mix$draws(c("alpha_pi","b_ecv","b_group","tau_sp_pi"),
                          format = "df") |>
  as_tibble()

p_mix_coef <- mix_coef |>
  select(b_ecv, b_group) |>
...
pi_s_summary <- fit_mix$draws("pi_s", format = "df") |>
  ...
  summarize(med = median(value),
            ...
            .groups = "drop") |>
  left_join(species_cov |> select(species_id, species),
            by = "species_id") |>
  arrange(med) |>
...
```{r post-pi-ind, fig.width=9, fig.height=5, fig.cap="Per-individual posterior probability of being a 1-ToM agent, colored by species. Individuals close to 0 or 1 are cleanly classified by their data; those clustered near their species π_s reflect the limited per-individual leverage of matching pennies."}
post_pi_draws <- fit_mix$draws("post_pi_j", format = "df") |>
  as_tibble() |>
  select(starts_with("post_pi_j")) |>
  pivot_longer(everything(), names_to = "param", values_to = "value") |>
...
```{r mix-occam-check, fig.width=7, fig.height=4.5, fig.cap="Per-individual posterior architecture probability (y) vs. per-individual 1-ToM minus 0-ToM log-likelihood difference (x), both extracted from the mixture fit. Posterior-mean points, colored by species. A monotone rising curve confirms the mixture is actually responsive to the structural evidence in each individual's choices; flat clouds would indicate the posterior is driven entirely by the species-level prior."}
ll_diff_summary <- fit_mix$draws("ll_diff_j", format = "df") |>
  as_tibble() |>
  select(starts_with("ll_diff_j")) |>
  pivot_longer(everything(), names_to = "param", values_to = "value") |>
  mutate(agent_id = as.integer(str_extract(param, "\\d+"))) |>
  group_by(agent_id) |>
...
```{r write-stan-hier-reg, cache=TRUE}
stan_tom0_reg <- "
data {
  int<lower=1> N;
  int<lower=1> J;
...
"
writeLines(stan_tom0_reg, here::here("stan", "ch8_tom0_reg.stan"))
mod_reg <- cmdstan_model(here::here("stan", "ch8_tom0_reg.stan"))
```
...
```{r fit-reg, cache=TRUE}
reg_path <- here::here("simmodels", "ch8_tom0_reg_fit.rds")
...
baseline_mu <- fit_hier$draws("mu_log_beta", format = "df") |>
  ...
  mutate(species_id = as.integer(str_extract(param, "\\d+"))) |>
  left_join(species_cov |> select(species_id, species), by = "species_id")
...
```{r stacking-crosscheck, eval=FALSE}
# Left as an exercise: fit mod_tom1_hier analogous to mod_tom0_hier,
# compute species-block stacking weights, compare to pi_s.
loo0 <- fit_hier$loo()
...
# Then scatter-plot w_tom1 (stacking) vs pi_s median (mixture).
```
