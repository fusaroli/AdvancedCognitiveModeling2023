...
```{tikz hierarchical_memory_dags, echo=FALSE, fig.cap="Standard Plate Notation (Left) and Kruschke-Style Generative Notation (Right) for the Hierarchical Memory Agent.", fig.ext='png', fig.width=12, cache=TRUE}
\usetikzlibrary{positioning, shapes.geometric, arrows.meta, fit, backgrounds, calc}

\begin{tikzpicture}[
    % GLOBAL STYLES
    stochastic/.style={->, >={Latex[length=3mm]}, thick},
    deterministic/.style={->, >={Latex[length=3mm]}, thick, dashed, draw=gray!80},
    formula/.style={align=left, font=\small},
    plate/.style={draw=black, thick, rounded corners, inner sep=12pt},
    ...
    % KRUSCHKE ICON DEFINITIONS
    pics/norm_dist/.style={
        code={
            \draw[thin, gray] (-1,0) -- (1,0);
            \draw[thick, blue] plot[domain=-1:1, samples=50] (\x, {0.6*exp(-3*\x*\x)});
            \node[font=\scriptsize, below] at (0,0) {Normal};
        }
    },
    pics/exp_dist/.style={
        code={
            \draw[thin, gray] (-0.2,0) -- (1.2,0);
            \draw[thick, blue] plot[domain=0:1.1, samples=50] (\x, {0.6*exp(-2.5*\x)});
    ...
    pics/lkj_dist/.style={
        code={
            \draw[thin, gray] (-1,0) -- (1,0);
            \draw[thick, blue] plot[domain=-0.9:0.9, samples=50] (\x, {0.6*(1-\x*\x)});
    ...
    pics/mvn_dist/.style={
        code={
            ...
            \draw[thick, blue, rotate=45] (0,0) ellipse (0.6 and 0.25);
            \draw[thick, blue, rotate=45] (0,0) ellipse (0.3 and 0.1);
            \node[font=\scriptsize, below right] at (0.2,-0.2) {MVN};
        }
    },
    pics/bern_dist/.style={
        code={
            \draw[thin, gray] (-0.8,0) -- (0.8,0);
            \draw[thick, blue, ycomb, line width=3pt] plot coordinates { (-0.3, 0.3) (0.3, 0.7) };
    ...
    % =========================================================
    % LEFT SIDE: STANDARD PLATE NOTATION
    % =========================================================
    \node[font=\large\bfseries] at (0, 2) {Standard Plate Notation};

    % 1. Population Level (Hyperparameters)
    \node[continuous, latent] (mu_a) at (-3, 0) {$\mu_\alpha$};
    \node[continuous, latent] (sigma_a) at (-1.5, 0) {$\sigma_\alpha$};
    \node[continuous, latent, fill=gray!10] (omega) at (0, 0) {$\Omega$};
    \node[continuous, latent] (sigma_b) at (1.5, 0) {$\sigma_\beta$};
    ...
    % 3. Data Level (Observed covariate and outcome)
    \node[discrete, observed] (m) at (-1.5, -7) {$m_{ij}$};
    \node[discrete, observed] (h) at (1.5, -7) {$h_{ij}$};

    % Plates
    \node[plate, fit=(m) (h), label={[anchor=south east, font=\small\bfseries]below right:$i=1 \dots N$}] (plate_N) {};
    \node[plate, inner sep=18pt, fit=(alpha) (beta) (plate_N), label={[anchor=south east, font=\small\bfseries]below right:$j=1 \dots J$}] (plate_J) {};

    % Edges (Stochastic)
    \draw[stochastic] (mu_a) -- (alpha);
    \draw[stochastic] (sigma_a) -- (alpha);
    \draw[stochastic] (omega) -- (alpha);
    \draw[stochastic] (omega) -- (beta);
    \draw[stochastic] (sigma_b) -- (beta);
    \draw[stochastic] (mu_b) -- (beta);
    
    \draw[stochastic] (alpha) -- (h);
    \draw[stochastic] (beta) -- (h);
    
    % Edges (Deterministic Covariate)
    \draw[deterministic] (m) -- (h);


    % =========================================================
    % RIGHT SIDE: KRUSCHKE-STYLE DISTRIBUTIONAL NOTATION
    % =========================================================
    \node[font=\large\bfseries] at (11, 2) {Kruschke-Style Notation};
    ...
    \coordinate (k_omega) at (11, 0); \draw (k_omega) pic {lkj_dist}; \node[above=0.6cm of k_omega, font=\bfseries] {$\Omega$ (Corr)};
    
    \coordinate (k_sig_b) at (13, 0); \draw (k_sig_b) pic {exp_dist}; \node[above=0.6cm of k_sig_b, font=\bfseries] {$\sigma_\beta$};
    \coordinate (k_mu_b) at (15, 0); \draw (k_mu_b) pic {norm_dist}; \node[above=0.6cm of k_mu_b, font=\bfseries] {$\mu_\beta$};

    % 2. Individual Level (Joint MVN)
    \coordinate (k_mvn) at (11, -3.5);
    \draw (k_mvn) pic {mvn_dist};
    \node[above=0.8cm of k_mvn, font=\bfseries] {Joint Agent Params $\begin{pmatrix} \alpha_j \\ \beta_j \end{pmatrix}$};

    % 3. Data Level
    \coordinate (k_m) at (9, -7);
    \node[discrete, observed, minimum size=0.8cm] (k_m_node) at (k_m) {$m_{ij}$};
    \node[above=0.5cm of k_m, font=\bfseries] {Opp. History};

    \coordinate (k_h) at (13, -7);
    \draw (k_h) pic {bern_dist};
    \node[above=0.8cm of k_h, font=\bfseries] {Choice ($h_{ij}$)};

    % Kruschke Plates
    \draw[plate, dashed] (8, -5) rectangle (14, -2) node[below left, font=\small\bfseries] {$j=1 \dots J$};
    \draw[plate, dotted] (8, -8) rectangle (14, -5.5) node[below left, font=\small\bfseries] {$i=1 \dots N$};

    % Edges
    \draw[stochastic] (k_mu_a) -- (k_mvn);
    \draw[stochastic] (k_sig_a) -- (k_mvn);
    \draw[stochastic] (k_omega) -- (k_mvn) node[midway, right, font=\large] {$\sim$};
    \draw[stochastic] (k_sig_b) -- (k_mvn);
    \draw[stochastic] (k_mu_b) -- (k_mvn);
    
    \draw[stochastic] (k_mvn) -- (k_h) node[midway, right, font=\large] {$\sim$};
    \draw[deterministic] (k_m_node) -- (12.5, -7);
...
```{tikz hierarchical_wsls_dags, echo=FALSE, fig.cap="Standard Plate Notation (Left) and Kruschke-Style Generative Notation (Right) for the Hierarchical WSLS Agent.", fig.ext='png', fig.width=12, cache=TRUE}
...
\begin{tikzpicture}[
    ...
    % LEFT SIDE: STANDARD PLATE NOTATION
    % =========================================================
    \node[font=\large\bfseries] at (0, 3) {Standard Plate Notation};

    % 1. Population Level (Hyperparameters)
    \node[continuous, latent] (mu_a) at (-4.5, 0) {$\mu_\alpha$};
    \node[continuous, latent] (sigma_a) at (-2.5, 0) {$\sigma_\alpha$};
    
    \node[continuous, latent, fill=gray!10] (omega) at (0, 2) {$\Omega$};
    
    \node[continuous, latent] (mu_bw) at (-1, 0) {$\mu_{\beta_W}$};
    \node[continuous, latent] (sigma_bw) at (1, 0) {$\sigma_{\beta_W}$};
    
    \node[continuous, latent] (mu_bl) at (2.5, 0) {$\mu_{\beta_L}$};
    \node[continuous, latent] (sigma_bl) at (4.5, 0) {$\sigma_{\beta_L}$};
    
    % 2. Individual Level
    \node[continuous, latent] (alpha) at (-3.5, -4) {$\alpha_j$};
    \node[continuous, latent] (beta_w) at (0, -4) {$\beta_{W,j}$};
    \node[continuous, latent] (beta_l) at (3.5, -4) {$\beta_{L,j}$};
    
    % 3. Data Level (Observed covariates and outcome)
    \node[discrete, observed] (win) at (-3, -8) {$W_{ij}$};
    \node[discrete, observed] (h) at (0, -8) {$h_{ij}$};
    \node[discrete, observed] (lose) at (3, -8) {$L_{ij}$};

    % Plates
    \node[plate, fit=(win) (h) (lose), label={[anchor=south east, font=\small\bfseries]below right:$i=1 \dots N$}] (plate_N) {};
    \node[plate, inner sep=18pt, fit=(alpha) (beta_w) (beta_l) (plate_N), label={[anchor=south east, font=\small\bfseries]below right:$j=1 \dots J$}] (plate_J) {};

    % Edges (Stochastic)
    \draw[stochastic] (mu_a) -- (alpha);
    \draw[stochastic] (sigma_a) -- (alpha);
    
    \draw[stochastic] (mu_bw) -- (beta_w);
    \draw[stochastic] (sigma_bw) -- (beta_w);
    
    \draw[stochastic] (mu_bl) -- (beta_l);
    \draw[stochastic] (sigma_bl) -- (beta_l);
    
    \draw[stochastic] (omega) -- (alpha);
    \draw[stochastic] (omega) -- (beta_w);
    \draw[stochastic] (omega) -- (beta_l);
    
    \draw[stochastic] (alpha) -- (h);
    \draw[stochastic] (beta_w) -- (h);
    \draw[stochastic] (beta_l) -- (h);
    
    % Edges (Deterministic Covariates)
    \draw[deterministic] (win) -- (h);
    \draw[deterministic] (lose) -- (h);


    % =========================================================
    % RIGHT SIDE: KRUSCHKE-STYLE DISTRIBUTIONAL NOTATION
    % =========================================================
    \node[font=\large\bfseries] at (12, 3) {Kruschke-Style Notation};

    % 1. Population Level
    \coordinate (k_mu_a) at (7, 0); \draw (k_mu_a) pic {norm_dist}; \node[above=0.6cm of k_mu_a, font=\bfseries] {$\mu_\alpha$};
    \coordinate (k_sig_a) at (9, 0); \draw (k_sig_a) pic {exp_dist}; \node[above=0.6cm of k_sig_a, font=\bfseries] {$\sigma_\alpha$};
    
    \coordinate (k_omega) at (12, 2); \draw (k_omega) pic {lkj_dist}; \node[above=0.6cm of k_omega, font=\bfseries] {$\Omega$ ($3 \times 3$ Corr)};
    ...
    % 2. Individual Level (Joint MVN)
    \coordinate (k_mvn) at (12, -4);
    \draw (k_mvn) pic {mvn_dist};
    \node[above=0.8cm of k_mvn, font=\bfseries, align=center] {Joint Agent Params \\ $\begin{pmatrix} \alpha_j \\ \beta_{W,j} \\ \beta_{L,j} \end{pmatrix}$};

    % 3. Data Level
    \coordinate (k_win) at (9, -8);
    \node[discrete, observed, minimum size=0.8cm] (k_win_node) at (k_win) {$W_{ij}$};
    \node[above=0.5cm of k_win, font=\bfseries] {Win Signal};

    \coordinate (k_h) at (12, -8);
    \draw (k_h) pic {bern_dist};
    \node[above=0.8cm of k_h, font=\bfseries] {Choice ($h_{ij}$)};
    
    \coordinate (k_lose) at (15, -8);
    \node[discrete, observed, minimum size=0.8cm] (k_lose_node) at (k_lose) {$L_{ij}$};
    \node[above=0.5cm of k_lose, font=\bfseries] {Lose Signal};

    % Kruschke Plates
    \draw[plate, dashed] (6, -6) rectangle (18, -2.5) node[below left, font=\small\bfseries] {$j=1 \dots J$};
    \draw[plate, dotted] (6, -9.5) rectangle (18, -6.5) node[below left, font=\small\bfseries] {$i=1 \dots N$};

    % Edges
    \draw[stochastic] (k_mu_a) -- (k_mvn);
    \draw[stochastic] (k_sig_a) -- (k_mvn);
    \draw[stochastic] (k_mu_bw) -- (k_mvn);
    \draw[stochastic] (k_sig_bw) -- (k_mvn);
    \draw[stochastic] (k_mu_bl) -- (k_mvn);
    \draw[stochastic] (k_sig_bl) -- (k_mvn);
    \draw[stochastic] (k_omega) -- (k_mvn) node[midway, right, font=\large] {$\sim$};
    
    \draw[stochastic] (k_mvn) -- (k_h) node[midway, right, font=\large] {$\sim$};
    \draw[deterministic] (k_win_node) -- (11.2, -8);
    \draw[deterministic] (k_lose_node) -- (12.8, -8);
