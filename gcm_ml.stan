...
data {
    int<lower=1> nsubject;  // number of trials
    int<lower=1> ntrials;  // number of trials
    int<lower=1> nfeatures;  // number of predefined relevant features
    array[ntrials, nsubjects] int<lower=0, upper=1> cat_one; // true responses on a trial by trial basis
    array[ntrials, nsubjects] int<lower=0, upper=1> y;  // decisions on a trial by trial basis
    array[ntrials, nfeatures, nsubjects] real obs; // stimuli as vectors of features
...
parameters {
    ...
    real<lower=0> kappa;
    array[nsubjects] simplex[nfeatures] w;    // weight parameter (how much attention should be paid to feature 1 related to feature 2 - summing up to 1)
}
      ...
      for (i in 1:ntrials) {
        ...
        for (e in 1:(i-1)){
            ...
            for (j in 1:nfeatures) {
                tmp_dist[j] = w[j]*abs(obs[e,j,k] - obs[i,j,k]);
            }
        ...
        if (sum(cat_one[:(i-1)])==0 || sum(cat_two[:(i-1)])==0){  // if there are no examplars in one of the categories
            ...
            // calculate r[i]
            rr[i,k] = (b*similarities[1]) / (b*similarities[1] + (1-b)*similarities[2]);

            // to make the sampling work
            if (rr[i,k] > 0.9999){
                r[i,k] = 0.9999;
            } else if (rr[i,k] < 0.0001) {
                r[i,k] = 0.0001;
            } else if (rr[i,k] > 0.0001 && rr[i,k] < 0.9999) {
                r[i,k] = rr[i,k];
            } else {
                r[i,k] = 0.5;
            }
