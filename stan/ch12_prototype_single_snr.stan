// Kalman Filter Prototype Model — Single Subject, SNR parameterisation
//
// Instead of the natural parameters (log_r, log_q) — which exhibit a
// strong negative-correlation ridge on stationary data because q and r
// both enter the steady-state decisional covariance — we parameterise
// via the observation noise and the drift-to-noise *ratio*:
//
//   log_r   : log observation noise
//   log_rho : log(q / r) — drift-to-noise ratio; the poorly-identified
//             direction under stationary data
//   log_q = log_r + log_rho (derived quantity)
//
// Under the SNR parameterisation, the two prior-independent parameters
// remain approximately prior-independent in the posterior, which makes
// MCMC geometry cleaner and SBC curves directly readable: a posterior
// that cannot pin down the direction of poor identifiability will now
// show it as a wide marginal on log_rho instead of a diagonal ridge
// spanning (log_r, log_q).
...
data {
  ...
  real prior_logrho_mean;
  real<lower=0> prior_logrho_sd;
}
parameters {
  real log_r;
  real log_rho;
